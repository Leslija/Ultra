	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Config.StopFishing) then
				endFishing()
				QBCore.Functions.Notify('Paras-te de pescar', 'error')
			end
			...
			if fishing then
				...
				if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z -3.0)  then
					...
					endFishing()
					QBCore.Functions.Notify('Água não é funda o suficiente para pescar', 'error')
				end
				if IsEntityDead(playerPed) or IsEntityInWater(playerPed) then
					endFishing()
					QBCore.Functions.Notify('A Pesca terminou', 'error')
				end
			end
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					QBCore.Functions.Notify('O Peixe escapou!', 'error')
					loseBait()
				end
			end
		end
...
CreateThread(function()
	while true do
		...
		if fishing then
			pause = true
			correct = 1
			QBCore.Functions.Notify('Pressiona [F] para apanhar o peixe!', 'error')
			input = 0
			pausetimer = 0
		end
	end
...
RegisterNetEvent('fishing:client:progressBar', function()
	exports['progressBars']:drawBar(1000, 'A abrir caixa de equipamentos')
end)
...
RegisterNetEvent('fishing:client:attemptTreasureChest', function()
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
		if HasItem then
			QBCore.Functions.Progressbar("accepted_key", "A inserir chaves..", (math.random(2000, 5000)), false, true, {
		  ...
		  QBCore.Functions.Notify("Não tens nenhuma chave para esta fechadura!", "error")
		end
	  end, 'fishingkey')
end)
...
RegisterNetEvent('fishing:client:useFishingBox', function(BoxId)
	TriggerServerEvent("inventory:server:OpenInventory", "stash", 'FishingBox_'..BoxId, {maxweight = 100000, slots = 250})
...
RegisterNetEvent('fishing:fishstart', function()
	...
	local pos = GetEntityCoords(playerPed) 
	if IsPedSwimming(playerPed) then return QBCore.Functions.Notify("Tu não podes nadar e pescar ao mesmo tempo.", "error") end 
	if IsPedInAnyVehicle(playerPed) then return QBCore.Functions.Notify("Tu precisas de sair do veículo para pescar.", "error") end 
	if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z - 3.0)  then
		local time = 1000
		QBCore.Functions.Notify('A usar cana de pesca', 'primary', time)
		Wait(time)
		QBCore.Functions.Notify('Pressiona [F] para apanhar o peixe!', 'primary')
		fishAnimation()
	else
		QBCore.Functions.Notify('Precisas de ir para mais longe da costa', 'error')
	end
end, false)
		...
		if isSuccess then 
			...
			if args == 1 then 
				QBCore.Functions.SpawnVehicle(Config.RentalBoat, function(boat)
					...
					exports['s_fuel']:SetFuel(boat, 100.0)
	...
	if data.location == 1 then 
        if Config.Fish24hours then
            ...
            if GetClockHours() >= 6 and GetClockHours() <= 21 then
                ...
                QBCore.Functions.Notify('Muito tarde para pescar! Aberto das 6 às 21!', 'error')
            end
        end
            ...
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                    if IsBoatAnchoredAndFrozen(currVeh) then
						QBCore.Functions.Notify('Âncora removida', 'success')
                        Wait(2000)
						QBCore.Functions.Notify('Âncora desativada', 'primary')
                    ...
                    elseif not IsBoatAnchoredAndFrozen(currVeh) and CanAnchorBoatHere(currVeh) and GetEntitySpeed(currVeh) < 3 then
						...
						QBCore.Functions.Notify('Lançando âncora', 'primary')
                        Wait(2000)
						QBCore.Functions.Notify('Âncora ativada', 'success')
...
loseBaitAnimation = function()
	...
	QBCore.Functions.Notify('Peixe mordeu a isca!', 'primary')
	Wait(2000)
	fishAnimation()
end
...
catchAnimation = function()
	...
	TaskPlayAnim(ped, animDict, animName, 1.0, -1.0, 1.0, 0, 0, 0, 48, 0)
	local time = 1750
	QBCore.Functions.Notify('Peixe apanhado!', 'success', time)
	Wait(time)
	TriggerServerEvent('fishing:server:catch') 
	loseBait()
...
fishAnimation = function()
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
		if HasItem then
		  ...
		  endFishing()
		  QBCore.Functions.Notify("Não tens isca de peixe", "error")
		end
	end, 'fishbait')
end
...
attemptTreasureChest = function()
	...
	RemoveAnimDict(animDict)
	QBCore.Functions.Notify('A tentar abrir o báu do tesouro!', 'primary', 1500)
	Wait(1500)
...
openedTreasureChest = function()
	if math.random(1,15) == 10 then
		...
		QBCore.Functions.Notify("A chave corroída quebrou", "error", 7500)
	end
	...
	QBCore.Functions.Notify("Baú do tesouro aberto! Certifique-se de levar tudo!!", "success", 7500)
	local ShopItems = {} 
	ShopItems.label = "Baú Tesouro"
