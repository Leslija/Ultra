...
QBCore.Functions.CreateUseableItem('raspadinha', function(source)
  local _source = source
  DebugPrint(('%s just used a scratching ticket.'):format(GetPlayerName(_source)))
  TriggerClientEvent("s_raspadinha:isActiveCooldown", source)
end)

RegisterNetEvent("s_raspadinha:handler", function(returncooldown, cooldown)
  ...
  local count <const> = xPlayer.Functions.GetItemByName('raspadinha').amount
  local randomNumber <const> = math.random(1, totalSumChance)
  local add = 0
  if returncooldown then
    if Config.ShowCooldownNotifications then
      TriggerClientEvent('QBCore:Notify', _source, 'Tens que esperar ' .. cooldown .. ' segundos!', 'error', cooldown > 30 and 5000 or cooldown * 1000)
    end
  ...
  if count >= 1 then
    xPlayer.Functions.RemoveItem('raspadinha', 1)
    DebugPrint(('Succesfully removed scratching ticket of %s (%s).'):format(playerName, playerIdentifier))
    TriggerClientEvent("s_raspadinha:setCooldown", _source)
    if Config.ShowUsedTicketNotification then
      TriggerClientEvent('QBCore:Notify', _source, 'Usas-te uma raspadinha com sucesso', 'success')
    end
  else
  ...
  TriggerClientEvent("s_raspadinha:startScratchingEmote", _source)
  ...
  for key,priceInfo in pairs(Config.Prices) do
    ...
    if randomNumber > add and randomNumber <= add + chance then
      ...
      TriggerClientEvent("s_raspadinha:nuiOpenCard", _source, key, price, amount, price_type, price_label)
      return price
    end
    add = add + chance
  end
...
RegisterNetEvent("s_raspadinha:deposit", function(key, price, amount, type)
  local _source = source
  local playerName, playerIdentifier = GetPlayerName(_source), GetPlayerIdentifier(_source, 0)
...
RegisterNetEvent("s_raspadinha:stopScratching", function(price, amount, type)
  local _source = source
  local playerName, playerIdentifier = GetPlayerName(_source), GetPlayerIdentifier(_source, 0)
  local tempsrc = tonumber(_source)
