...
$(function() {
  window.addEventListener('message', function(event) {
    if (event.data.type === "openScratch") {
      ...
      if(price_type == 'money') {
        formattedPrice = Number.parseFloat(price).toFixed(0) + ' ' + currency; // Want decimals? Change 0 -> n of decimals
        price > 0 ? document.getElementById('price').innerHTML = "<span style='color:#00b6c3'>" + win_message + "</span><br><br><span style='font-size:50px;'>" + formattedPrice + '</span>' : document.getElementById('price').innerHTML = "<span style='color:#B2BABB;text-transform:uppercase;'>" + lose_message + "</span><br><br><span style='font-size:60px;'>0 " + currency + "</span>";
      } else {
        document.getElementById('price').innerHTML = "<span style='color:#00b6c3'>" + win_message + "</span><br><span style='font-size:20px;color:#7F8C8D'>" + amount + "x</span><br><span style='font-size:40px;'>" + price + '</span>'
      }
