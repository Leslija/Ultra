...
local start = false
discord = {
    ['webhook'] = 'https://discord.com/api/webhooks/977568677836775454/b4ph2a8nZM3T6wGbxLlGntHToIzOtQ5ZG8Z_TTbnOT4j-vf7W2aSH2z0iayODuBVAF7K',
    ['name'] = '2LoveRP',
...
QBCore.Functions.CreateCallback('pacificheist:server:checkTime', function(source, cb)
    ...
    if (os.time() - lastrob) < Config['PacificHeist']['nextRob'] and lastrob ~= 0 then
        ...
        lastrob = os.time()
        start = true
        discordLog(player.PlayerData.name ..  ' -  começou Assalto ao banco principal!')
        TriggerEvent('qb-scoreboard:server:SetActivityBusy', "principal", true)
        cb(true)
    end
end)
...
RegisterCommand('pdpacific', function(source, args)
    ...
    if player then
        if (player.PlayerData.job.name == "police" and player.PlayerData.job.onduty) then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Não pertences à policia!", "error")
        end
    end
end)
