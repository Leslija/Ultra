...
AddEventHandler('kidnapping:client:policeAlert', function(targetCoords)
    QBCore.Functions.Notify(Strings['police_alert'], "police")
    local alpha = 250
        ...
        if KidNapping['sceneFinish'] then
            ...
            if cameraDist <= 3.0 then
                ...
                if IsControlJustPressed(0, 38) then
                    ...
                    QBCore.Functions.Notify(Strings['go_laptop'], "primary")
                end
            end
        end
...
local query = false
function CloseEyes()
    QBCore.Functions.TriggerCallback('kidnapping:server:checkQueryRoom', function(status)
        if not status then
            query = false
            ...
            QBCore.Functions.Notify(Strings['query_room_busy'], "primary")
        end
    end)
end

function KidnappingStart()
    QBCore.Functions.TriggerCallback('kidnapping:server:checkTime', function(time)
        if time then
            TriggerServerEvent("raptossim")
            query = true
            QBCore.Functions.Notify(Strings['info_1'], "primary")
            QBCore.Functions.Notify(Strings['info_2'], "primary")
            QBCore.Functions.Notify(Strings['info_3'], "primary")
            local ped = PlayerPedId()
            ...
            Citizen.CreateThread(function()
                while query do
                    ...
                    if dist2 >= 150.0 then
                        QBCore.Functions.Notify(Strings['mission_failed2'], "primary")
                        break
                    end
                    ...
                    if IsEntityPlayingAnim(KidNapping['kidnappedPed'], 'missminuteman_1ig_2', 'handsup_enter', 1) then
                        ...
                        if GetEntityType(y) == 2 then
                            ...
                            ShowHelpNotification("Vai para a sala de tortura.") 
                            ...
                            if not DoesBlipExist(queryBlip) then
                                queryBlip = addBlip(Config['Kidnapping']['query']['scenePos'], 364, 0, 'Query Room')
                            end
                        end
                    end
                    ...
                    if IsEntityDead(KidNapping['kidnappedPed']) then
                        ...
                        QBCore.Functions.Notify(Strings['mission_failed'], "error")
                        break
                    end
...
function Blindfold()
    if not attached then
        ...
        if dist <= 2.0 then
            ...
            QBCore.Functions.Notify(Strings['cant_blindfold'], "error")
        end
    end
end
