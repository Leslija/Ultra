...
AddEventHandler('pacificheist:client:policeAlert', function(targetCoords)
	QBCore.Functions.Notify(Strings['police_alert2'], "error")
    local alpha = 250
    ...
    QBCore.Functions.TriggerCallback('pacificheist:server:checkPoliceCount', function(status)
        if status then
            QBCore.Functions.TriggerCallback('pacificheist:server:checkTime', function(time)
                if time then
					QBCore.Functions.Notify(Strings['start_heist'], "error")
					QBCore.Functions.Notify(Strings['start_heist2'], "error")
...
function LaptopHack(index)
    QBCore.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem)
        if hasItem then
...
function PaintingScene(sceneId)
    QBCore.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem)
        if hasItem then
            ...
            if weapon ~= GetHashKey('WEAPON_SWITCHBLADE') then ShowNotification(Strings['need_switchblade']) QBCore.Functions.Notify(Strings['need_switchblade'], "error") return end
