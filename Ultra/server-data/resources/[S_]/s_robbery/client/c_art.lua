                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            if weapon == GetHashKey('WEAPON_SWITCHBLADE') then
                                if not ArtHeist['cuting'] then
                                    ...
                                    QBCore.Functions.Notify(Strings['already_cuting'], 'error')
                                end
                            else
                                QBCore.Functions.Notify('Não tens uma switchblade', 'error')
                            end
                        end
                    end
                end
...
AddEventHandler('artheist:client:policeAlert', function(targetCoords)
    ...
    local alpha = 250
    local artBlip = AddBlipForRadius(1405.61, 1144.92, 114.33, 50.0)

    SetBlipHighDetail(artBlip, true)
    SetBlipColour(artBlip, 1)
    SetBlipAlpha(artBlip, alpha)
    SetBlipAsShortRange(artBlip, true)
...
function StartHeist()
    QBCore.Functions.TriggerCallback('artheist:server:checkRobTime', function(time)
        if time then
            if ArtHeist['start'] then QBCore.Functions.Notify(Strings['already_heist'], "error") return end
         ...
         --   TriggerServerEvent("arteestado")
            TriggerServerEvent('artheist:server:policeAlert', pedCo)
...
function HeistAnimation(sceneId)
    ...
    if ArtHeist['cut'] == 1 then
       TriggerEvent("artheist:client:policeAlert")
    end
