...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('QBCore:Notify', src, "Não tens os itens necessários..", "error")
		end
	elseif fromInventory == "attachment_crafting" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			TriggerClientEvent("QBCore:Notify", src, "Item não existe?", "error")
		end
	end
end)
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
	...
	if Player == OtherPlayer then return TriggerClientEvent('QBCore:Notify', src, "Não podes dar a ti próprio um item") end
	if dist > 2 then return TriggerClientEvent('QBCore:Notify', src, "Estás muito longe!") end
	...
	if item.name ~= name then TriggerClientEvent('QBCore:Notify', src, "Erro, tenta novamente!"); return end
	...
	if amount <= item.amount then
		...
		if Player.Functions.RemoveItem(item.name, amount, item.slot) then
			if OtherPlayer.Functions.AddItem(item.name, amount, false, item.info) then
				...
				TriggerClientEvent('QBCore:Notify', target, "Tu recebeste "..amount..' '..item.label.." de "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
				TriggerClientEvent('inventory:client:ItemBox',src, QBCore.Shared.Items[item.name], "remove")
				TriggerClientEvent('QBCore:Notify', src, "Tu deste " .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."!")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('qb-inventory:client:giveAnim', src)
				TriggerClientEvent('qb-inventory:client:giveAnim', target)
			else
				Player.Functions.AddItem(item.name, amount, item.slot, item.info)
				TriggerClientEvent('QBCore:Notify', src,  "O inventário está cheio!", "error")
				TriggerClientEvent('QBCore:Notify', target,  "O teu inventário está cheio!", "error")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
			end
		else
			TriggerClientEvent('QBCore:Notify', src,  "Não tens quantidade suficiente", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Não tens items suficientes para dar")
	end
end)
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if dist < 3.0 then
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Nome:</strong> {1} <br><strong>Apelido:</strong> {2} <br><strong>Data de Nascimento:</strong> {3} <br><strong>Licenças:</strong> {4}</div></div>',
					args = {
						"Carta Condução",
						item.info.firstname,
						item.info.lastname,
...
QBCore.Functions.CreateUseableItem("id_card", function(source, item)
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if dist < 3.0 then
			...
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Civ ID:</strong> {1} <br><strong>Nome:</strong> {2} <br><strong>Apelido:</strong> {3} <br><strong>Data de Nascimento:</strong> {4} <br><strong>Sexo:</strong> {5} <br><strong>Nacionalidade:</strong> {6}</div></div>',
					args = {
						"Cartão de Cidadão",
						item.info.citizenid,
						item.info.firstname,
