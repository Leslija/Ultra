...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.GetPlayerData(function(PlayerData) PlayerGang = PlayerData.gang
        if PlayerData.gang then
            for k, v in pairs(Config.GangsArmas) do if v == PlayerGang.name then havejob = true end end
            if havejob then TriggerServerEvent("QBCore:ToggleDuty") end end
    end)
end)

RegisterNetEvent('QBCore:Client:OnJobUpdate') AddEventHandler('QBCore:Client:OnJobUpdate', function(JobInfo) PlayerGang = JobInfo onDuty = PlayerGang.onduty end)
...
function jobChecks()
    ...
    if Config.RequiredJob == true then check = false
        for k, v in pairs(Config.GangsArmas) do
            if v == PlayerGang.name then check = true end
        end
        if check == false then QBCore.Functions.Notify("Não tens permissão", "error") check = false end
    end
    return check
end
function jobChecks2()
    local check = true
    if Config.RequiredJob == true then check = false
        for k, v in pairs(Config.GangsAccessorios) do
            if v == PlayerGang.name then check = true end
        end
        if check == false then QBCore.Functions.Notify("Não tens permissão", "error") check = false end
    end
...
RegisterNUICallback('showBlur', function()
    Wait(50)
    TriggerEvent("s_inventory:client:showBlur")
end) 
RegisterNetEvent("s_inventory:client:showBlur", function()
    Wait(50)
...
CreateThread(function()
	while true do
		...
		local inRange = false
		local distance = #(pos - Config.CraftArmas)
		if distance < 10 then
			inRange = true
			if distance < 1.5 then
				DrawText3Ds(Config.CraftArmas.x, Config.CraftArmas.y, Config.CraftArmas.z, "~g~E~w~ - Craft Armas")
...
CreateThread(function()
	while true do
		local pos = GetEntityCoords(PlayerPedId())
		local inRange = false
		local distance = #(pos - Config.CraftAcessorios)
		if distance < 10 then
			inRange = true
			if distance < 1.5 then
				DrawText3Ds(Config.CraftAcessorios.x, Config.CraftAcessorios.y, Config.CraftAcessorios.z, "~g~E~w~ - Craft Acessórios")
				if IsControlJustPressed(0, 38) then
					local crafting = {}
					crafting.label = "Craft Accessórios"
					crafting.items = GetAttachmentThresholdItems()
                    if not jobChecks2() then return end
