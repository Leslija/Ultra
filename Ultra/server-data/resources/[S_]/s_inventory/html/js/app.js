...
function showBlur() {
    $.post('https://s_inventory/showBlur');
}
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://s_inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://s_inventory/RemoveAttachment",
                JSON.stringify({
    ...
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            $(".item-info-description").html(
                ...
                "</span></p><p><strong>Sexo: </strong><span>" +
                gender +
                "</span></p><p><strong>Nacionalidade: </strong><span>" +
            ...
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html(
                "<p><strong>Cartão Advogado: </strong><span>" +
                itemData.info.id +
                "</span></p><p><strong>Nome: </strong><span>" +
                itemData.info.firstname +
                "</span></p><p><strong>Apelido: </strong><span>" +
            ...
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p>Espécie: ' + itemData.info.species + '</p>Peso: ' + itemData.info.lbs + ' kgs</p>Tipo: ' + itemData.info.type);
            ...
            $(".item-info-description").html('<p><strong>Dono: </strong><span>' + itemData.info.boxOwner + '</span></p> Para guardar os peixes');
        } else if (itemData.name == "harness") {
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html(
                "<p>" + itemData.info.uses + " usos restantes.</p>"
            );
            ...
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html(
                "<p><strong>Quantidade de Dinheiro: </strong><span>$" +
                itemData.info.cash +
                "</span></p>"
            );
            ...
            var cardNumber = "************" + res;
            $(".item-info-description").html('<p><strong>Nome: </strong><span>' + itemData.info.name + '</span></p><p><strong>Citizen ID: </strong><span>' + itemData.info.citizenid + '</span></p><p><strong>Nº Cartão: </strong><span>' + cardNumber + '</span></p>');
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://s_inventory/combineWithAnim",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://s_inventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://s_inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://s_inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                if (
                    ...
                    $.post(
                        "https://s_inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://s_inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $.post("https://s_inventory/CloseInventory", JSON.stringify({}));
    ...
    Inventory.itemBox = function(data) {
        ...
        var type = "Usou " + data.itemAmount + "x";
        if (data.type == "add") {
          type = "Recebeu " + data.itemAmount + "x";
        } else if (data.type == "remove") {
          type = "Removido " + data.itemAmount + "x";
        }
