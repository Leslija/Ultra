...
AddEventHandler('aspect_paycheck:menu:Open_menu', function()
     QBCore.Functions.TriggerCallback('aspect_paycheck:server:account_information', function(result)
          if result then
               Open_menu(result)
               return
          end
...
local function withdraw_all(maximum)
     ...
     QBCore.Functions.TriggerCallback('aspect_paycheck:server:withdraw_all', function(result, reason)
          if result then
               Speach('thanks')
...
local function withdraw_amount(maximum)
     ...
     if inputData then
          ...
          QBCore.Functions.TriggerCallback('aspect_paycheck:server:withdraw_amount', function(result, reason)
               if result then
                    TriggerEvent('animations:client:EmoteCommandStart', { "ID" })
                    QBCore.Functions.Notify(Lang:t('success.successful_withdraw') .. reason .. '$', "success")
                    Speach('thanks')
...
function menu:withdraw_menu(data)
     ...
     elseif req == 3 then
          ...
          QBCore.Functions.TriggerCallback('aspect_paycheck:server:get_logs', function(result)
...
function QbMenu:withdraw_menu(data)
     ...
     local Menu = {
          ...
          },
          {
               ...
               icon = 'fa-solid fa-money-bill-transfer',
               params = {
                    event = 'aspect_paycheck:client:function_caller',
          ...
          },
          {
               ...
               params = {
                    event = "aspect_paycheck:client:close_menu",
               }
          }
     }
...
AddEventHandler('aspect_paycheck:client:function_caller', function(data)
     TriggerEvent('animations:client:EmoteCommandStart', { "c" })
     if data.id == 1 then
          -- Withdraw All
          withdraw_all(data.money)
...
AddEventHandler('aspect_paycheck:client:close_menu', function()
     TriggerEvent('qb-menu:closeMenu')
end)

AddEventHandler('qb-menu:closeMenu', function()
     if not GetCurrentResourceName() == 'aspect_paycheck' then
          return
     end
     TriggerEvent('animations:client:EmoteCommandStart', { "c" })
end)
