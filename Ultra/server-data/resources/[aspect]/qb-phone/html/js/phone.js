                ...
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (QB.Phone.Data.AnonymousCall) {
                            QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Começas-te uma chamada anonima");
                        }
                    ...
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Esta pessoa esta ocupada!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Esta pessoa esta indisponivel!");
            }
        } else {
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            if (contact.name == '') contact.name = 'Hmm, Não deveria conseguir fazer isto...'
            var ContactElement = '<div class="phone-contact" data-contactid="'+i+'"><div class="phone-contact-firstletter" style="background-color: whitesmoke;">'+'<i style="color: rgb(44, 70, 95); font-size:2.4vh; margin-top:15%;" class="fas fa-user"></i>'+'</div><div class="phone-contact-name">'+contact.name+'</div><div class="phone-contact-action-buttons"> <i class="fas fa-phone" id="phone-start-call"data-toggle="tooltip" data-placement="left" title="Ligar"></i> <i class="fas fa-comment" id="new-chat-phone" style="font-size: 2.5vh;"data-toggle="tooltip" data-placement="left" title="Mensagem"></i> <i class="fas fa-clipboard" id="edit-contact"data-toggle="tooltip" data-placement="left" title="Editar"></i> <i class="fas fa-trash" id="delete-contact"data-toggle="tooltip" data-placement="left" title="Apagar"></i> </div></div>'
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Telefone", "Não podes mandar mensagens a ti proprio..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    if (CurrentEditContactData.name == '') CurrentEditContactData.name = 'Hmm, não devias conseguir fazer isto...'
...
$(document).on('click', '#phone-number-savecontact-edit', function(e){
    ...
    if (ContactName == '') ContactName = 'Hmm, não devias conseguir fazer isto...'
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Editar contato", "Preenche todos os campos!");
    }
});
...
$(document).on('click', '#phone-number-savecontact', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Add Contacto", "Preenche todos os campos!");
    }
});
        ...
        if (cData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Esta pessoa esta um uma chamada!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Esta pessoa esta indisponivel!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Telefone", "Não pode ligar a ti proprio!");
        }
    });
}
    ...
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            ...
            setTimeout(function(){
                ...
                if (AnonymousCall) {
                    Label = "Estas a receber uma chamada anonima"
                }
                $(".phone-call-outgoing").css({"display":"none"});
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        if (cData.CallType == "incoming") {
            ...
            $(".phone-currentcall-title").html("A ligar");
        } else if (cData.CallType == "ongoing") {
            $(".phone-currentcall-title").html("A ligar ("+cData.CallTime+")");
        }

        $(".phone-currentcall-contact").html("De "+cData.TargetData.name);
    } else {
...
QB.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
    ...
    if (AmountOfSuggestions > 0) {
        $(".amount-of-suggested-contacts").html(AmountOfSuggestions + " contactos");
