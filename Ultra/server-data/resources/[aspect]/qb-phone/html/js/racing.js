...
$(document).on('click', '.racing-race', function(e){
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corridas", "Corrida já começou..", "#1DA1F2");
    }
});
                ...
                if (InDistance) {
                    $.post('https://qb-phone/IsBusyCheck', JSON.stringify({
                        ...
                        if (!IsBusy) {
                            ...
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corridas", "Estas no editor..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corridas", "Já estas numa corrida..", "#1DA1F2");
        }
    });
});
            ...
            if (!data.IsBusy) {
                $.post('https://qb-phone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Estas numa corrida..", "#1DA1F2");
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Já estas a fazer uma pista..", "#1DA1F2");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Não tens permissões para isto..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        ...
        if (TrackName == '') TrackName = 'Que estas a tentar?'
        $.post('https://qb-phone/IsAuthorizedToCreateRaces', JSON.stringify({
            ...
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Nome nao esta disponivel..", "#1DA1F2");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Não tens permissões para isto..", "#1DA1F2");
            }
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Coloca um nome..", "#1DA1F2");
    }
});
                        ...
                        if (laps !== "") {
                            $.post('https://qb-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    ...
                                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Não a nenhuma ..", "#1DA1F2");
                                }
                            });
                        } else {
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Preenche as voltas..", "#1DA1F2");
                        }
                    } else {
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Não selecionas-te uma pista..", "#1DA1F2");
                    }
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Corrida", "Já tens um corrida activa..", "#1DA1F2");
        }
    });
});
