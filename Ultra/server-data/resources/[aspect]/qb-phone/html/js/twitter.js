...
$(document).on('click', '#twt-sendmessage-chat', function(e){
    ...
    if (TweetMessage != "") {
        ...
        QB.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Coloca uma mensagem!", "#1DA1F2");
    };
    $('#tweet-new-url').val("");
});
...
QB.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            if (clean == '') clean = 'Hmm, Não devia conseguir fazer isto...'
            ...
            if (Tweet.url == "") {
                let TweetElement = '<div class="twitter-tweet" data-twtcid="'+Tweet.citizenid+'" data-twtid ="'+Tweet.tweetId+'" data-twthandler="@' + TwitterHandle.replace(" ", "_") + '"><div class="tweet-reply" data-toggle="tooltip" data-placement="left" title="Responder"><i class="fas fa-reply"></i></div>' +
                ...
                let TweetElement = '<div class="twitter-tweet" data-twthandler="@'+TwitterHandle.replace(" ", "_")+'"><div class="tweet-reply"data-toggle="tooltip" data-placement="left" title="Responder"><i class="fas fa-reply"></i></div>'+
                    '<div class="tweet-tweeter">'+Tweet.firstName+' '+Tweet.lastName+' &nbsp;<span>@'+TwitterHandle.replace(" ", "_")+' &middot; '+TimeAgo+'</span></div>'+
                    '<div class="tweet-message">'+TwtMessage+'</div>'+
...
QB.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    ...
    if (Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            if (clean == '') clean = 'Hmm, Não devias conseguir fazer isto...'
            var TwtMessage = QB.Phone.Functions.FormatTwitterMessage(clean);
            var TimeAgo = moment(Tweet.date).format('MM/DD/YYYY hh:mm');
