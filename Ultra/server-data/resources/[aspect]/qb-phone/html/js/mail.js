...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            ...
            $(".mail-list").html('<p class="nomails">Não tens nenhum Mail..</p>');
        }

    }
}

var MonthFormatting = ["Janeiro", "Febreiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"];
...
$(document).on('click', '#advert-sendmessage-chat', function(e){
    ...
    if (Advert !== "") {
        ...
        QB.Phone.Notifications.Add("fas fa-ad", "Anuncios", "Coloca uma mensagem!", "#ff8f1a", 2000);
    }
});
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $.each(Adverts, function(i, advert){
            ...
            if (clean == '') { clean = 'Sou mesmo parvo :/' }
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">Nenhuma anuncio!</span></div>';
        $(".advert-list").append(element);
    }
}
...
$(document).on('click','#adv-delete',function(e){
    ...
    $.post('https://qb-phone/DeleteAdvert', function(){
        setTimeout(function(){
            QB.Phone.Notifications.Add("fas fa-ad", "Advertisement", "Anuncio apagado", "#ff8f1a", 2000);
        },400)
    });
})