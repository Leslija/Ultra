...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.Notify("Não tens um telemovel", "error")
        end
    end)
end
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Telefone",
                text = "Não estas a receber uma chamada...",
                icon = "fas fa-phone",
                color = "#e84118",
            },
        })
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            QBCore.Functions.Notify("Ação indisponivel de momento..", "error")
        end
    end
end)
...
if findVehFromPlateAndLocate(veh.plate) then
    QBCore.Functions.Notify("Veiculo foi marcado", "success")
else
    QBCore.Functions.Notify("Veiculo não pode ser localizado", "error")
end
end)
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('qb-phone:server:ScanPlate', function(result)
	    ...
	    if QBCore.Shared.Vehicles[vehname] ~= nil then
                ...
                result.label = 'Marca desconhecida..'
            end
            cb(result)
        end, plate)
    end, plate)
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Estas longe da corrida. GPS colocado.', 'error', 5000)
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Banco", text = "&#36;"..amount.." adicionado a tua conta!", icon = "fas fa-university", color = "#8c7ae6", }, })
...
RegisterNetEvent('qb-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Mail",
            text = "Novo Mail de: "..MailData.sender,
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Faturas',
            subject = 'Fatura paga',
            message = 'Fatura de '..name..' no montante'..data.amount..'€ foi paga',
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Faturas',
            subject = 'Fatura rejeitada',
            message = 'Fatura de '..name..' no montante '..data.amount.. '€ foi rejeitada',
        })
    end
end)
...
RegisterNUICallback('phone-silent-button', function(data,cb)
    if CallVolume == tonumber("0.2") then
        CallVolume = 0
        QBCore.Functions.Notify("Modo silencioso ON", "success")
        cb(true)
    else
        CallVolume = 0.2
        QBCore.Functions.Notify("Modo silencioso Off", "error")
        cb(false)
    end
end)
...
RegisterNetEvent('qb-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Nova menção!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
