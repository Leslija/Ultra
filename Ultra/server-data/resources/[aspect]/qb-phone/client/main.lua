...
local function PublicPhone()
    local PublicPhoneobject = {
        ...
        exports["s_target"]:AddTargetModel(PublicPhoneobject, {
            options = {
                {
                    type = "client",
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            QBCore.Functions.Notify("Ação nao disponivel de momento..", "error")
        end
    end
end)

RegisterKeyMapping('phone', 'Abrir telemovel', 'keyboard', 'M')
...
if findVehFromPlateAndLocate(veh.plate) then
    QBCore.Functions.Notify("Veiculo localizado", "success")
else
    QBCore.Functions.Notify("Veiculo nao pode ser localizado", "error")
end
end)
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    QBCore.Functions.Notify('GPS: '..data.alert.title)
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Demasiado longe da corrida. GPS colocado.', 'error', 5000)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("GPS colocado para " .. data.HouseData.HouseData.adress .. "!", "success")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS colocado!', 'success')
end)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
		...
		SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Banco", text = "Conta não existe!", icon = "fas fa-university", color = "#ff0000", }, })
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            elseif ChatType == "location" then
                PhoneData.Chats[NumberKey].messages[ChatKey].messages[#PhoneData.Chats[NumberKey].messages[ChatKey].messages+1] = {
                    message = "Localização partilhada",
                    time = ChatTime,
                    sender = PhoneData.PlayerData.citizenid,
                    type = ChatType,
                    data = {
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then
            QBCore.Functions.TriggerCallback("qb-phone:server:GetWebhook",function(hook)
                QBCore.Functions.Notify('A preparar foto...', 'primary')
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                    ...
                    QBCore.Functions.Notify('Foto guardada!', 'success')
                    OpenPhone()
                end)
            end)
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Banco", text = "&#36;"..amount.." add a tua conta!", icon = "fas fa-university", color = "#8c7ae6", }, })
    ...
    if not delete then 
        if src ~= MyPlayerId then
            SendNUIMessage({
                action = "PhoneNotification",
                PhoneNotify = {
                    title = "Novo tweet: (@"..NewTweetData.firstName.." "..NewTweetData.lastName..")",
                    text = "Novo tweet foi publicado.",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                },
            })
...
RegisterNetEvent('qb-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Mail",
            text = "Novo email de: "..MailData.sender,
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Departamento Faturas',
            subject = 'Fatura paga',
            message = 'Fatura foi paga por '..name..' no montante de '..data.amount..'€',
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Departamento Faturas',
            subject = 'Fatura foi rejeitada',
            message = 'Fatura rejeitada por '..name..' no montante de '..data.amount..'€',
        })
    end
end)
...
RegisterNUICallback('phone-silent-button', function(data,cb)
    if CallVolume == tonumber("0.2") then
        CallVolume = 0
        QBCore.Functions.Notify("Som OFF", "success")
        cb(true)
    else
        CallVolume = 0.2
        QBCore.Functions.Notify("Som ON", "error")
        cb(false)
    end
end)
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Mensagens",
                        text = "Nova mensagem: "..IsNumberInContacts(SenderNumber),
                        icon = "fas fa-comment",
                        color = "#25D366",
                        timeout = 1500,
                    },
...
RegisterNetEvent('qb-phone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        QBCore.Functions.Notify("Ninguem perto!", "error")
    end
end)
...
RegisterNetEvent('qb-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Mencionaram-te!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
...
RegisterNetEvent('qb-pings:client:SendLocation', function(PingData, SenderData)
    QBCore.Functions.Notify('Localização colocada no mapa', 'success')
    ...
    SetTimeout(5 * (60 * 1000), function()
        QBCore.Functions.Notify('Ping '..PingData.sender..' expirou...', 'error')
