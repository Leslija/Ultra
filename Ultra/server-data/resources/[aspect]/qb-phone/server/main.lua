...
local WebHook = "https://discordapp.com/api/webhooks/709385620706951219/TXbHn04lHgRW_H0xqho0B0xAGEGXB5K212T_92zWy87pM_mv2uS50W13cc4EYwCutCDE"
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        invoiceMailData = {
            ...
            message = string.format('Recebes-te uma comissão de €%s quando %s %s Pagou uma fatura de $%s.', commission, Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
...
QBCore.Functions.CreateCallback('qb-phone:server:ScanPlate', function(source, cb, plate)
    ...
    if plate ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Sem veiculos perto', 'error')
        cb(nil)
    end
end)
...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Numero de conta nao foi encontrado!", "error")
    end
end)
...
QBCore.Commands.Add('fatura', 'Enviar Fatura', {{name = 'id', help = 'ID Jogador'}, {name = 'amount', help = 'Montante fatura'}}, false, function(source, args)
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', source, 'Tem de ser um valor acima de 0', 'error')
                end
            else
                TriggerClientEvent('QBCore:Notify', source, 'Não podes mandar faturas a ti proprio...', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Jogador nao esta online', 'error')
        end
    else
...
RegisterNetEvent('qb-pings:server:acceptping', function()
    ...
    if Pings[src] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', Pings[src].sender, Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname.." Aceitou o pedido!")
        Pings[src] = nil
    else
        TriggerClientEvent('QBCore:Notify', src, "Não tens Ping's...", "error")
    end
end)
...
RegisterNetEvent('qb-pings:server:denyping', function()
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Pings[src] ~= nil then
        TriggerClientEvent('QBCore:Notify', Pings[src].sender, "Pedido de ping rejeitado...", "error")
        TriggerClientEvent('QBCore:Notify', src, "Rejeitas-te o Ping...", "success")
        Pings[src] = nil
    else
        TriggerClientEvent('QBCore:Notify', src, "Não tens Ping's...", "error")
    end
...
RegisterNetEvent('qb-pings:server:SendPing', function(id)
    ...
    if Target ~= nil then
        ...
        if OtherItem ~= nil then
            TriggerClientEvent('QBCore:Notify', src, "Pedis-te a localização de  "..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname)
            Pings[id] = {
                coords = coords,
                sender = src,
            }
            TriggerClientEvent('QBCore:Notify', id, "Recebes-te um pedido de Ping de "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname..". Use the app to allow or reject!")
            TriggerClientEvent('qb-phone:ping:client:UiUppers', id, true)
        else
            TriggerClientEvent('QBCore:Notify', src, "Não foi possivel enviar...", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Jogador não esta na cidade...", "error")
    end
end)
...
QBCore.Commands.Add("p#", "Dar numero telemovel", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if dist < 3.0 then
            TriggerClientEvent('chat:addMessage', v,  {
                template = '<div class="chat-message" style="background-color: rgba(234, 135, 23, 0.50);">Numero : <b>{0}</b></div>',
                args = {number}
            })
        end
    end
    ...
    if casino_status then
        if Player.PlayerData.money.bank >= amount then
            if not CasinoBetList[CSN] then
                Player.Functions.RemoveMoney('bank', amount, "Aposta casino")
                CasinoBetList[CSN] = {['csn'] = CSN, ['amount'] = amount, ['player'] = data.player, ['chanse'] = data.chanse, ['id'] = data.id}
            else
                TriggerClientEvent('QBCore:Notify', src, "Já estas a apostar...", "error")
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "Não tens dinheiro suficiente!", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Apostas nao estao ativas...", "error")
    end
end)
...
RegisterNetEvent('qb-phone:server:DeleteAndClearTable', function()
    ...
    BetNumber = 0
    TriggerClientEvent('qb-phone:client:addbetForAll', -1, CasinoTable)
    TriggerClientEvent('QBCore:Notify', src, "Feito", "success")
end)
    ...
    for k, v in pairs(CasinoBetList) do
        if v.id == Winer then
            ...
            if OtherPly then
                ...
                OtherPly.Functions.AddMoney('bank', tonumber(amount), "Vencedor Casino")
            end
        end
    end
end)
...
RegisterNetEvent('qb-phone:server:SetJobJobCenter', function(data)
    ...
    if Player.Functions.SetJob(data.job, 0) then
        TriggerClientEvent('QBCore:Notify', src, 'Mudas-te o trabalho para: '..data.label)
    else
        TriggerClientEvent('QBCore:Notify', src, 'Trabalho invalido...', 'error')
    end
end)
...
RegisterNetEvent('qb-phone:server:employment_CreateJobGroup', function(data)
    ...
    if Player then
        if not EmploymentGroup[CSN] then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Já crias-te um grupo...", "error")
        end
    end
end)
    ...
    for k, v in pairs(EmploymentGroup) do
        for ke, ve in pairs(v['UserName']) do
            if ve == data.PCSN then
                TriggerClientEvent('QBCore:Notify', src, "Já te juntas-te a um grupo!", "error")
                return
            end
        end
        table.insert(EmploymentGroup[data.id]['UserName'], data.PCSN)
        EmploymentGroup[data.id]['Users'] = v.Users + 1
        TriggerClientEvent('QBCore:Notify', src, "Juntaste-te ao grupo!", "success")
        TriggerClientEvent('qb-phone:client:EveryoneGrupAddsForAll', -1, EmploymentGroup)
        return
    end
end)
...
RegisterNetEvent('qb-phone:server:SendBillForPlayer_debt', function(data)
    ...
    if billed then
            if amount and amount > 0 then
                ...
                TriggerClientEvent('QBCore:Notify', src, 'Divida enviada!', 'success')
                TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, 'Nova divida recebida')
                Wait(1)
                TriggerClientEvent('qb-phone:RefreshPhoneForDebt', billed.PlayerData.source)
            else
                TriggerClientEvent('QBCore:Notify', src, 'Tem de ser um valor acima de 0', 'error')
            end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Jogador não esta online', 'error')
    end
end)
...
RegisterNetEvent('qb-phone:server:debit_AcceptBillForPay', function(data)
    ...
    if OtherPly then
        if Ply.PlayerData.money.bank then
            Ply.Functions.RemoveMoney('bank', Amount, "Remover dinheiro")
            OtherPly.Functions.AddMoney('bank', Amount,"Adicionar dinheiro")
...
RegisterNetEvent('qb-phone:server:documents_Save_Note_As', function(data)
    ...
    if data.Type == "New" then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Nota guardada')
    elseif data.Type == "Update" then
        ...
        if Note[1] ~= nil then
            ...
            MySQL.insert('INSERT INTO phone_note (citizenid, title,  text, lastupdate) VALUES (?, ?, ?, ?)',{Ply.PlayerData.citizenid, data.Title, data.Text, data.Time})
            TriggerClientEvent('QBCore:Notify', src, 'Nota actualizada', 'success')
        end
    elseif data.Type == "Delete" then
        local ID = tonumber(data.ID)
        MySQL.query('DELETE FROM phone_note WHERE id = ?', {ID})
        TriggerClientEvent('QBCore:Notify', src, 'Nota apagada', 'error')
    end
    Wait(100)
