...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        invoiceMailData = {
            sender = 'Departamentos Faturas',
            subject = 'Comissão recebida',
            message = string.format('Recebes-te uma comissao de %s€ quando %s %s pagou a fatura.', commission, Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    elseif not SenderPly and Config.BillingCommissions[society] then
        invoiceMailData = {
            sender = 'Departamentos Faturas',
            subject = 'Fatura paga',
            message = string.format('%s %s pagou a fatura', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    end
...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Numero de conta não existe!", "error")
    end
end)
...
QBCore.Commands.Add('bill', 'Faturas', {{name = 'id', help = 'ID'}, {name = 'amount', help = 'Montante'}}, false, function(source, args)
            ...
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    MySQL.insert(
                        ...
                        {billed.PlayerData.citizenid, amount, biller.PlayerData.job.label,
                    ...
                    TriggerClientEvent('QBCore:Notify', source, 'Montante tem de ser acima de 0', 'error')
                end
            else
...
RegisterNetEvent('qb-pings:server:acceptping', function()
    ...
    if Pings[src] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', Pings[src].sender, Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname.." Aceite!")
        Pings[src] = nil
    else
        TriggerClientEvent('QBCore:Notify', src, "Sem Pings...", "error")
    end
end)
...
RegisterNetEvent('qb-pings:server:denyping', function()
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Pings[src] ~= nil then
        TriggerClientEvent('QBCore:Notify', Pings[src].sender, "Pedido foi rejeitado...", "error")
...
RegisterNetEvent('qb-pings:server:SendPing', function(id)
    ...
    if Target ~= nil then
        ...
        if OtherItem ~= nil then
            TriggerClientEvent('QBCore:Notify', src, "Pedis-te a localização a "..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname)
            Pings[id] = {
                coords = coords,
                sender = src,
            }
            TriggerClientEvent('QBCore:Notify', id, "Recebes-te um ping de "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname..". Usa a app para rejeitar!")
            TriggerClientEvent('qb-phone:ping:client:UiUppers', id, true)
        else
            TriggerClientEvent('QBCore:Notify', src, "Error...", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Pessoa fora da cidade...", "error")
    end
end)
    ...
    if casino_status then
        if Player.PlayerData.money.bank >= amount then
            if not CasinoBetList[CSN] then
                Player.Functions.RemoveMoney('bank', amount, "Apostas")
        ...
        TriggerClientEvent('QBCore:Notify', src, "Apostas desativadas...", "error")
    end
end)
    ...
    for k, v in pairs(CasinoBetList) do
        if v.id == Winer then
            ...
            if OtherPly then
                ...
                OtherPly.Functions.AddMoney('bank', tonumber(amount), "Vencedor")
            end
        end
    end
end)
...
RegisterNetEvent('qb-phone:server:SetJobJobCenter', function(data)
    ...
    if Player.Functions.SetJob(data.job) then
        TriggerClientEvent('QBCore:Notify', src, 'Mudas-te de trabalho para: '..data.label)
    else
    ...
    for k, v in pairs(EmploymentGroup) do
        for ke, ve in pairs(v['UserName']) do
            if ve == data.PCSN then
                TriggerClientEvent('QBCore:Notify', src, "Já te juntas-te ao grupo!", "error")
                return
            end
        end
...
RegisterNetEvent('qb-phone:server:SendBillForPlayer_debt', function(data)
    ...
    if billed then
            if amount and amount > 0 then
                ...
                TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, 'Divida recebida')
                Wait(1)
                TriggerClientEvent('qb-phone:RefreshPhoneForDebt', billed.PlayerData.source)
            else
                TriggerClientEvent('QBCore:Notify', src, 'Montante tem de ser acima de 0', 'error')
            end
    else
        TriggerClientEvent('QBCore:Notify', src, 'ID não existe', 'error')
    end
end)
...
RegisterNetEvent('qb-phone:server:documents_Save_Note_As', function(data)
    ...
    if data.Type == "New" then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Guardada')
    elseif data.Type == "Update" then
        ...
        if Note[1] ~= nil then
            ...
            MySQL.insert('INSERT INTO phone_note (citizenid, title,  text, lastupdate) VALUES (?, ?, ?, ?)',{Ply.PlayerData.citizenid, data.Title, data.Text, data.Time})
            TriggerClientEvent('QBCore:Notify', src, 'Actualizada', 'success')
        end
    elseif data.Type == "Delete" then
        local ID = tonumber(data.ID)
        MySQL.query('DELETE FROM phone_note WHERE id = ?', {ID})
        TriggerClientEvent('QBCore:Notify', src, 'Apagada', 'error')
    end
    Wait(100)
