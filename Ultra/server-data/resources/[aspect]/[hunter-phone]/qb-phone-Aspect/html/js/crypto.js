...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Carteira: "+CryptoData.Portfolio+" bit('s)");
    ...
    if (CryptoData.History.length > 0) {
        ...
        $.each(CryptoData.History, function(i, change){
            ...
            var Element =   '<div class="cryptotab-course-block">' +
                                ...
                                '<span class="cryptotab-course-block-title">Mudança Valor</span>' +
...
function RefreshCryptoTransactions(data) {
    ...
    if (data.CryptoTransactions.length > 0) {
        ...
        $.each(data.CryptoTransactions, function(i, transaction){
            ...
            if (transaction.TransactionTitle == "Vendido" || transaction.TransactionTitle == "Transferido") {
...
$(document).on('click', '.cryptotab-general-action', function(e){
    e.preventDefault();
    $(".crypto-buy-ammount-for-update").html("0 Euros");
...
$(".crypto-buy-input").keyup(function(){
    ...
    $(".crypto-buy-ammount-for-update").html(MoneyAmount+" Euros");
});
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (QB.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://qb-phone/BuyCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "Banco", "&#36; "+Price+",- Foi retirado do teu banco!", "#badc58", 2500);
                    $('#crypto-buy-btn').fadeOut(350);
                } else {
                    QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "Não tens dinheiro suficiente..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "Não tens dinheiro suficiente..", "#badc58", 1500);
        ...
        QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "Preenche todos os campos!", "#badc58", 1500);
    }
});
        ...
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://qb-phone/SellCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    QB.Phone.Data.PlayerData.money.bank = parseInt(QB.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    QB.Phone.Notifications.Add("fas fa-university", "Banco", "&#36; "+Price+",- Foi adicionado a teu banco!", "#badc58", 2500);
                    $('#crypto-sell-btn').fadeOut(350);
                } else {
                    QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "Não tens Bit's suficientes..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "Não tens Bit's suficientes..", "#badc58", 1500);
            ...
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://qb-phone/TransferCrypto', JSON.stringify({
                    ...
                    if (CryptoData == "notenough") {
                        QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "Não tens bits suficientes..", "#badc58", 1500);
                    } else if (CryptoData == "notvalid") {
                        QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Esta carteira não existe!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Tranferis-te "+Coins+",- para "+WalletId+"!", "#badc58", 2500);
                        $('#crypto-transfer-btn').fadeOut(350);
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Não podes transferir para ti proprio..", "#badc58", 2500);
            }
        } else {
            QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "Não tens bits suficientes..", "#badc58", 1500);
        }
