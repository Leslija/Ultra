        ...
        if src ~= MyPlayerId then
            SendNUIMessage({
                ...
                PhoneNotify = {
                    ...
                    text = "New tweet, as it was published.",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                },
            })
            ...
            SendNUIMessage({
                ...
                PhoneNotify = {
                    title = "Twitter",
                    text = "The tweet has been published!",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                    timeout = 1000,
                },
...
RegisterNetEvent('qb-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Mail",
            text = "New email from: "..MailData.sender,
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Advertisement",
            text = "New announcement published: "..LastAd,
            icon = "fas fa-ad",
            color = "#ff8f1a",
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Invoicing department',
            subject = 'invoice paid',
            message = 'The invoice was paid by '..name..' In the amount of $'..data.amount,
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Invoicing department',
            subject = 'The invoice has been rejected',
            message = 'The invoice has been rejected '..name..' In the amount of $'..data.amount,
        })
    end
end)
...
RegisterNUICallback('phone-silent-button', function(data,cb)
    if CallVolume == tonumber("0.2") then
        CallVolume = 0
        QBCore.Functions.Notify("The sound is off", "success")
        cb(true)
    else
        CallVolume = 0.2
        QBCore.Functions.Notify("Sound is on", "error")
        cb(false)
    end
end)
...
RegisterNetEvent('qb-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "They mentioned to you!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
