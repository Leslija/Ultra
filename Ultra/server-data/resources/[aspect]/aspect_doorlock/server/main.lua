...
lib.versionCheck('overextended/aspt_doorlock')
lib.locale()
...
exports('editDoor', function(id, data)
	...
	if door then
		...
		MySQL.update('UPDATE aspt_doorlock SET name = ?, data = ? WHERE id = ?', { door.name, encodeData(door), id })
		TriggerClientEvent('aspt_doorlock:editDoorlock', -1, id, door)
	end
end)
...
local function createDoor(id, door, name)
	...
	if type(door.items?[1]) == 'string' then
		...
		door.items = items
		MySQL.update('UPDATE aspt_doorlock SET data = ? WHERE id = ?', { encodeData(door), id })
	end
...
MySQL.ready(function()
	...
	local success, result = pcall(MySQL.query.await, 'SELECT id, name, data FROM aspt_doorlock')

	if not success then
		-- because some people can't run sql files
		MySQL.query([[CREATE TABLE `aspt_doorlock` (
...
RegisterNetEvent('aspt_doorlock:setState', function(id, state, lockpick, passcode)
	local door = doors[id]
	...
	if door then
		...
		if authorised then
			door.state = state
			TriggerClientEvent('aspt_doorlock:setState', -1, id, state, source)
			...
			if door.autolock and state == 0 then
				SetTimeout(door.autolock * 1000, function()
					if door.state ~= 1 then
						...
						TriggerClientEvent('aspt_doorlock:setState', -1, id, door.state)
						TriggerEvent('aspt_doorlock:stateChanged', nil, door.id, door.state == 1)
					end
				end)
			end

			return TriggerEvent('aspt_doorlock:stateChanged', source, door.id, state == 1, type(authorised) == 'string' and authorised)
		end
	end
...
RegisterNetEvent('aspt_doorlock:getDoors', function()
	local source = source
	while not isLoaded do Wait(100) end
	TriggerClientEvent('aspt_doorlock:setDoors', source, doors, sounds)
end)

RegisterNetEvent('aspt_doorlock:editDoorlock', function(id, data)
	if IsPlayerAceAllowed(source, 'command.doorlock') then
		...
		if id then
			if data then
				MySQL.update('UPDATE aspt_doorlock SET name = ?, data = ? WHERE id = ?', { data.name, encodeData(data), id })
			else
				MySQL.update('DELETE FROM aspt_doorlock WHERE id = ?', { id })
			end

			doors[id] = data
			TriggerClientEvent('aspt_doorlock:editDoorlock', -1, id, data)
		else
			local insertId = MySQL.insert.await('INSERT INTO aspt_doorlock (name, data) VALUES (?, ?)', { data.name, encodeData(data) })
			local door = createDoor(insertId, data, data.name)

			TriggerClientEvent('aspt_doorlock:setState', -1, door.id, door.state, false, door)
		end
	end
end)

lib.addCommand(Config.CommandPrincipal, 'doorlock', function(source, args)
	TriggerClientEvent('aspt_doorlock:triggeredCommand', source, args.edit)
