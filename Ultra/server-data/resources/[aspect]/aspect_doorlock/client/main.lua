lib.locale()
TriggerServerEvent('aspt_doorlock:getDoors')
...
RegisterNetEvent('aspt_doorlock:setDoors', function(data, sounds)
	doors = data
...
RegisterNetEvent('aspt_doorlock:setState', function(id, state, source, data)
	if data then
		doors[id] = data
		createDoor(data)
	end
		...
		if closestDoor and closestDoor.distance < closestDoor.maxDistance then
			...
			if IsDisabledControlJustReleased(0, 38) then
				if closestDoor.passcode then
					...
					if input then
						TriggerServerEvent('aspt_doorlock:setState', closestDoor.id, closestDoor.state == 1 and 0 or 1, false, input[1])
					end
				else
					...
					if gameTimer - lastTriggered > 500 then
						...
						TriggerServerEvent('aspt_doorlock:setState', closestDoor.id, closestDoor.state == 1 and 0 or 1)
					end
				end
			end
		elseif showUI then
