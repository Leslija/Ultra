...
local function pickLock(entity)
	...
	while success == nil do
		...
		if math.random(1, 500) == 1 then
			TriggerServerEvent('aspt_doorlock:breakLockpick')
			lib.cancelProgress()
	...
	if success then
		TriggerServerEvent('aspt_doorlock:setState', door.id, door.state == 1 and 0 or 1, true)
	end
...
do
	if GetResourceState('s_target'):find('start') then
		target = exports['s_target']
		rawset(target, 'name', 's_target')
	else
...
local function removeDoorlock(entity)
	...
	if door then
		return TriggerServerEvent('aspt_doorlock:editDoorlock', door.id)
	end
end
...
local function newDoorlock()
	...
	TriggerServerEvent('aspt_doorlock:editDoorlock', false, data)
	table.wipe(tempData)
...
local function editDoorlock(entity)
	...
	if door then
		...
		SetNuiFocus(false, false)
		table.wipe(tempData)
		TriggerServerEvent('aspt_doorlock:editDoorlock', data.id, data)
	end
end

local function removeTarget(res)
	if not res or res == 'aspt_doorlock' then
...
RegisterNetEvent('aspt_doorlock:triggeredCommand', function(edit)
	if edit then
