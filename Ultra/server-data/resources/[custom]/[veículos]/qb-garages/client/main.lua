    ...
    if HouseGarages[house] then     
        ...
        Polyzone:onPlayerInOut(function(isPpointInside)
            if isPpointInside then
                inHouseParking = true
                exports['qb-core']:DrawText('Garagem','left')
            else
...
local function MenuHouseGarage(house)
    exports['qb-menu']:openMenu({
        ...
        },
        {
            ...
            txt = "Ver os teus veículos guardados!",
            params = {
...
RegisterNetEvent('vehicle:flipit', function()
	...
	QBCore.Functions.Progressbar("flipping_vehicle", "Virar veículo", 15000, false, true, {
...
RegisterNetEvent("qb-garages:client:DepotList", function(data)
    ...
    QBCore.Functions.TriggerCallback("qb-garage:server:GetDepotVehicles", function(result)
        if result == nil then
            QBCore.Functions.Notify("Não tens nenhum veículo apreendido", "error", 5000)
        else
            local MenuDepotOptions = {
                {
                    header = "Apreendidos: "..Depots[currentGarage].label,
                    isMenuHeader = true
                },
            }
            for k, v in pairs(result) do
                ...
                MenuDepotOptions[#MenuDepotOptions+1] = {
                    ...
                    txt = "Matrícula: "..v.plate.."<br>Gas: "..currentFuel.." | Motor: "..enginePercent.." | Carroçaria: "..bodyPercent,
                    params = {
...
RegisterNetEvent('qb-garages:client:takeOutDepot', function(vehicle)
    ...
    if not VehExists then
        if not IsSpawnPointClear(vector3(Depots[currentdepot].spawnPoint[lastnearspawnpoint].x, Depots[currentdepot].spawnPoint[lastnearspawnpoint].y, Depots[currentdepot].spawnPoint[lastnearspawnpoint].z), 2.5) then
            QBCore.Functions.Notify("O veículo está a caminho", 'error', 2500)
            return
        else
                ...
                if OutsideVehicles[vehicle.plate] then
                    ...
                    QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                        QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                            ...
                            exports['s_fuel']:SetFuel(veh, vehicle.fuel)
        ...
        QBCore.Functions.Notify("O teu veículo nãop está nos apreendidos", "error", 5000)
    end
end)
...
RegisterNetEvent("qb-garages:client:HouseGarage", function(house)
    ...
    QBCore.Functions.TriggerCallback("qb-garage:server:GetHouseVehicles", function(result)
        if result == nil then
            QBCore.Functions.Notify("Não tens nenhum veículo na garagem!", "error", 5000)
        else
            local MenuHouseGarageOptions = {
                {
                    header = "Garagem: "..HouseGarages[house].label,
                    isMenuHeader = true
                },
            }
            ...
            for k, v in pairs(result) do
                ...
                elseif v.state == 2 then
                    v.state = "Apreendido Pela PSP"
                end

                MenuHouseGarageOptions[#MenuHouseGarageOptions+1] = {
                    header = vname.." ["..v.plate.."]",
                    txt = "Estado: "..v.state.. "<br>Gas: "..currentFuel.." | Motor: "..enginePercent.." | Carroçaria: "..bodyPercent,
                    params = {
...
RegisterNetEvent('qb-garages:client:TakeOutHouseGarage', function(vehicle)
    if vehicle.state == "Guardado" then
        QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
            QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                QBCore.Functions.SetVehicleProperties(veh, properties)
    ...
    QBCore.Functions.TriggerCallback("qb-garage:server:GetUserVehicles", function(result)
        if result == nil then
            ...
            local MenuPublicGarageOptions = {
                {
                    header = "Garagem: "..Garages[currentGarage].label,
                    isMenuHeader = true
                },
            }
            for k, v in pairs(result) do
...
RegisterNetEvent('qb-garages:client:takeOutPublicGarage', function(vehicle)
    ...
    elseif vehicle.state == "Fora" then
        QBCore.Functions.Notify("O teu veículo deve estar nos apreendidos!", "error", 2500)
    elseif vehicle.state == "Apreendido" then
        QBCore.Functions.Notify("Este veículo foi apreendido pela policia!", "error", 4000)
    end
end)
...
RegisterNetEvent('qb-garages:client:putGarageHouse', function()
    ...
    QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleHouseOwner', function(owned)
        if owned then
            ...
            local totalFuel = exports['s_fuel']:GetFuel(curVeh)
            local vehProperties = QBCore.Functions.GetVehicleProperties(curVeh)
            CheckPlayers(curVeh)
            if DoesEntityExist(curVeh) then
                QBCore.Functions.Notify("O Veículo não foi guardado, verifica se está alguem dentro dele", "error", 4500)
            else
            ...
            QBCore.Functions.Notify("Veículo Guardado", "primary", 4500)
        end
        else
            QBCore.Functions.Notify("Não és o dono deste veículo", "error", 3500)
        end
...
RegisterNetEvent('qb-garages:client:openHouseGarage', function()
    if hasGarageKey then
        ...
        QBCore.Functions.Notify("Não podes abrir esta garagem, não tens a chave.")
    end
end)
        ...
        if inParkingZone then
            ...
            QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                if owned then
                    ...
                    QBCore.Functions.Notify("Veículo guardado em "..Garages[currentgarage].label, "primary", 5000)
                else
                    QBCore.Functions.Notify("Não és dono deste veiculo", "error", 3500)
                end
            end, plate)
        end
    end
...
hayesdepot:onPlayerInOut(function(isPointInside)
    if isPointInside then
        ...
        exports['qb-core']:DrawText('Apreendidos','left')
    else
        inDepotZone = false
        exports['qb-radialmenu']:RemoveOption(5)
        exports['qb-core']:HideText()	
