        ...
        if vehicles then
            for i = 1, #vehicles, 1 do
                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['s_target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = "client",
...
local function despawnOccasionsVehicles()
    ...
    for i = 1, #oSlot, 1 do
        ...
        if EntityZones[i] and Config.UseTarget then
            exports['s_target']:RemoveZone(EntityZones[i])
        end
    end
    EntityZones = {}
end
            ...
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        ...
                        QBCore.Functions.Notify("Não estás em nenhum veículo!", "error", 4500)
                    end
                end
            end
            Wait(3)
...
RegisterNetEvent('qb-vehiclesales:client:OpenContract', function(data)
    ...
    if CurrentVehicle then
        ...
        QBCore.Functions.Notify('Este veículo não está à venda!!', 'error', 7500)
    end
end)
    ...
    for _, cars in pairs(Config.Zones) do
        ...
        SpawnZone[_]:onPlayerInOut(function(isPointInside)
            if isPointInside and IsPedInAnyVehicle(PlayerPedId(), false) then
                exports['qb-core']:DrawText('[E] Vender Veículo', 'left')
                TextShown = true
                Listen4Control()
            else
                listen = false
            ...
            for k, v in pairs(Config.Zones[_].VehicleSpots) do
                ...
                VehicleZones:onPlayerInOut(function(isPointInside)
                    if isPointInside and IsCarSpawned(k) then
                        exports['qb-core']:DrawText('[E] Ver Contrato', 'left')
                        TextShown = true
                        Listen4Control(k)
                    else
                        listen = false
