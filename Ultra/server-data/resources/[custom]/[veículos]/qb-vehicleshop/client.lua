...
local vehHeaderMenu = {
    {
        header = 'Opções Veículo',
        icon = "fa-solid fa-car",
        txt = 'Interagir com o veículo',
        params = {
    ...
    CreateThread(function()
        while inTestDrive do
            if GetGameTimer() < gameTimer + tonumber(1000 * testDriveTime) then
                ...
                drawTxt('Test Drive Tempo Restante: '..math.ceil(testDriveTime - secondsLeft / 1000), 4, 0.5, 0.93, 0.50, 255, 255, 255, 180)
            end
            Wait(0)
        end
    end)
...
local function createVehZones(shopName, entity)
    if not Config.UsingTarget then
        ...
        exports['s_target']:AddTargetEntity(entity, {
            options = {
                {
                    type = "client",
                    event = "qb-vehicleshop:client:showVehOptions",
                    icon = "fas fa-car",
                    label = "Interagir Veículo",
            ...
            CreateThread(function()
                while insideShop do
                    ...
                    vehicleMenu = {
                        ...
                        {
                            header = 'Test Drive',
                            txt = 'Test drive veículo selecionado',
                        ...
                        {
                            header = "Comprar Veículo",
                            txt = 'Comprar veículo selecionado',
                        ...
                        },
                        {
                            ...
                            header = 'Trocar Veículo',
                            txt = 'Trocar o veículo selecionado',
                            params = {
...
RegisterNetEvent('qb-vehicleshop:client:TestDrive', function()
    if not inTestDrive and ClosestVehicle ~= 0 then
        ...
        QBCore.Functions.SpawnVehicle(Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle, function(veh)
            ...
            exports['s_fuel']:SetFuel(veh, 100)
            ...
            testDriveVeh = veh
            QBCore.Functions.Notify('Tu tens '..Config.Shops[closestShop]["TestDriveTimeLimit"]..' minutos restantes')
            SetTimeout(Config.Shops[closestShop]["TestDriveTimeLimit"] * 60000, function()
                if testDriveVeh ~= 0 then
                    ...
                    QBCore.Functions.Notify('Test Drive completado!')
                end
            end)
        ...
        QBCore.Functions.Notify('Já estás em test drive!', 'error')
    end
end)
...
RegisterNetEvent('qb-vehicleshop:client:customTestDrive', function(data)
    if not inTestDrive then
        ...
        QBCore.Functions.SpawnVehicle(vehicle, function(veh)
            ...
            TriggerServerEvent('qb-vehicletuning:server:SaveVehicleProps', QBCore.Functions.GetVehicleProperties(veh))
            testDriveVeh = veh
            QBCore.Functions.Notify('Tu tens '..Config.Shops[insideShop]["TestDriveTimeLimit"]..' minutos restantes')
            SetTimeout(Config.Shops[insideShop]["TestDriveTimeLimit"] * 60000, function()
                if testDriveVeh ~= 0 then
                    ...
                    QBCore.Functions.DeleteVehicle(veh)
                    SetEntityCoords(PlayerPedId(), prevCoords)
                    QBCore.Functions.Notify('Teste drive completado')
                end
            end)
...
RegisterNetEvent('qb-vehicleshop:client:TestDriveReturn', function()
    ...
    if veh == testDriveVeh then
        ...
        QBCore.Functions.Notify('Este não é o teu veículo de teste drive!', 'error')
    end
end)
...
RegisterNetEvent('qb-vehicleshop:client:swapVehicle', function(data)
    ...
    if Config.Shops[shopName]["ShowroomVehicles"][data.ClosestVehicle].chosenVehicle ~= data.toVehicle then
        ...
        SetVehicleNumberPlateText(veh, 'TESTE')
