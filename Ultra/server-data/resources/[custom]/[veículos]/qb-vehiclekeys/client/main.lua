        ...
        if LocalPlayer.state.isLoggedIn then
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                    if IsEntityDead(driver) then
                        if not isTakingKeys then
                            ...
                            QBCore.Functions.Progressbar("steal_keys", "A tirar as chaves...", 2500, false, false, {
            ...
            if IsPedInAnyVehicle(ped, false) and not IsHotwiring then
                ...
                if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() and not HasKeys(plate) and not isBlacklistedVehicle(vehicle) then
                    ...
                    DrawText3D(vehiclePos.x, vehiclePos.y, vehiclePos.z, "~g~[H]~w~ - Procurar por chaves")
...
RegisterKeyMapping('togglelocks', 'Trancar Veiculo', 'keyboard', 'L')
...
RegisterNetEvent('qb-vehiclekeys:client:GiveKeys', function(id)
    ...
    if targetVehicle then
        ...
        if HasKeys(targetPlate) then
            ...
            QBCore.Functions.Notify("Não tens as chaves deste veículo.", 'error')
        end
    end
end)
...
function GiveKeys(id, plate)
    ...
    if distance < 1.5 and distance > 0.0 then
        ...
        QBCore.Functions.Notify('Ninguem por perto para dar as chaves.','error')
    end
end
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(QBCore.Functions.GetPlate(veh)) then
                ...
                if vehLockStatus == 1 then
                    ...
                    QBCore.Functions.Notify("Veículo Trancado!", "primary")
                else
...
function lockpickFinish(success)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            ...
            QBCore.Functions.Notify('Conseguis-te abrir a porta!', 'success')
...
function Hotwire(vehicle, plate)
    ...
    QBCore.Functions.Progressbar("hotwire_vehicle", "A procurar pelas chaves...", hotwireTime, false, true, {
        ...
        if (math.random() <= Config.HotwireChance) then
            ...
            QBCore.Functions.Notify("Não conseguiste encontrar as chaves e ficas-te frustado.", "error")
        end
...
function AttemptPoliceAlert(type)
    if not AlertSend then
        ...
        if math.random() <= chance then
           TriggerServerEvent('police:server:policeAlert', 'Roubo de veículo em progresso, tipo: ' .. type)
        end
        AlertSend = true
