    ...
    while true do
        ...
        for k in pairs(Config.Registers) do
            ...
            if dist <= 1 and Config.Registers[k].robbed then
                inRange = true
                DrawText3Ds(Config.Registers[k][1].xyz, 'A caixa registadora está vazia')
            end
        end
        if not inRange then
            Wait(2000)
                ...
                if dist < 3 then
                    ...
                    if dist < 1.0 then
                        if not Config.Safes[safe].robbed then
                            DrawText3Ds(Config.Safes[safe][1].xyz, '~g~E~w~ - Tentar Combinação')
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    QBCore.Functions.Notify("Não existe polícia suficiente", "error")
                                end
                            end
                        else
                            DrawText3Ds(Config.Safes[safe][1].xyz, 'Cofre Aberto')
                        end
                    end
                end
            end
...
RegisterNUICallback('fail', function(_ ,cb)
    ...
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        QBCore.Functions.Notify("Partis-te a lockpick")
    end
    lockpick(false)
    cb('ok')
end)
...
RegisterNetEvent('qb-storerobbery:client:robberyCall', function(_, _, _, coords)
    if PlayerJob.name == "police" and onDuty then
        ...
        AddTextComponentString("10-31 | Assalto a Loja")
