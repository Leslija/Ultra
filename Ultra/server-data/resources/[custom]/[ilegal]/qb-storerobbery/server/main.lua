...
RegisterNetEvent('qb-storerobbery:server:takeMoney', function(register, isDone)
	...
	Player.Functions.AddItem('black_money', 25000)
        if math.random(1, 100) <= 10 then
            local code = SafeCodes[Config.Registers[register].safeKey]
            if Config.Safes[Config.Registers[register].safeKey].type == "keypad" then
                info = {
                    label = "Código Cofre: "..tostring(code)
                }
            else
                info = {
                    label = "Código Cofre: "..tostring(math.floor((code[1] % 360) / 3.60)).."-"..tostring(math.floor((code[2] % 360) / 3.60)).."-"..tostring(math.floor((code[3] % 360) / 3.60)).."-"..tostring(math.floor((code[4] % 360) / 3.60)).."-"..tostring(math.floor((code[5] % 360) / 3.60))
                }
            end
...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function()
	...
	Player.Functions.AddItem('black_money', 50000)
...
RegisterNetEvent('qb-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    ...
    local alertData = {
        title = "10-33 | Assalto a Loja",
        coords = {x = coords.x, y = coords.y, z = coords.z},
        description = "Assalto a loja"
    }
