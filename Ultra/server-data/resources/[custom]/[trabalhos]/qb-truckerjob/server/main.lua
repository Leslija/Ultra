...
RegisterNetEvent('qb-trucker:server:DoBail', function(bool, vehInfo)
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Pagas-te 250€ de caução.', 'success')
        ...
        elseif Player.PlayerData.money.bank >= Config.BailPrice then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Precisas de 250€ para a caução.', 'error')
        end
    else
        if Bail[Player.PlayerData.citizenid] ~= nil then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], "trucker-bail-paid")
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('QBCore:Notify', src, 'Recebes-te os 250€ da caução.', 'success')
        end
    end
end)
...
RegisterNetEvent('qb-trucker:server:01101110', function(drops)
    ...
    local bonus = 0
    local DropPrice = math.random(120, 130)
  ...
  --  Player.Functions.AddJobReputation(drops)
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Tu ganhas-te '..payment..'€', 'success')
end)

RegisterNetEvent('qb-trucker:server:nano', function()
    local xPlayer = QBCore.Functions.GetPlayer(tonumber(source))
	xPlayer.Functions.AddItem("penoffshore", 1, false)
	TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["penoffshore"], "add")
end)
