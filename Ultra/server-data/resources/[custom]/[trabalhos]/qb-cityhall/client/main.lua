...
local function spawnPeds()
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            ...
            if opts then
                exports['s_target']:AddTargetEntity(ped, {
                    options = {opts},
                    distance = 2.0
                })
            end
                    ...
                    if isLoggedIn and closestCityhall and closestDrivingSchool then
                        if inside then
                            if current.drivingschool then
                                ...
                                exports['qb-core']:DrawText('[E] Carta de Condução')
                            elseif current.cityhall then
                                inRangeCityhall = true
                                exports['qb-core']:DrawText('[E] Centro de Emprego')
                            end
                        else
...
RegisterNUICallback('close', function(_, cb)
    ...
    if not Config.UseTarget and inRangeCityhall then exports['qb-core']:DrawText('[E] Centro de Emprego') end -- Reopen interaction when you're still inside the zone
    cb('ok')
end)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        ...
        QBCore.Functions.Notify(('Recebes-te o teu %s '):format(license.label, id.cost), 'success', 3500)
    else
