	...
	for k, v in pairs(Config.JobRoles) do if v == biller.PlayerData.job.name then havejob = true end end if havejob then
		if billed ~= nil then
				...
				if billtype == "cash" then balance = billed.Functions.GetMoney(billtype)
					if balance >= amount then billed.Functions.RemoveMoney('cash', amount) TriggerEvent("qb-bossmenu:server:addAccountMoney", tostring(biller.PlayerData.job.name), amount)	TriggerEvent('jim-mechanic:Tickets', tostring(biller.PlayerData.job.name), amount)
					else 
