...
AddEventHandler("mdt:getOffenderDetailsById", function(char_id)
    ...
    MySQL.Async.fetchAll('SELECT * FROM `players` WHERE `id` = ?', {char_id}, function(result)
        ...
        if not offender then
            ...
            TriggerClientEvent("mdt:sendNotification", usource, "Esta pessoa não existe.")
            return
        end
...
AddEventHandler("mdt:saveOffenderChanges", function(id, changes, identifier)
	...
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = ?', {id}, function(result)
		...
		TriggerClientEvent("mdt:sendNotification", usource, "As alterações foram guardadas.")
	end)
end)
...
AddEventHandler("mdt:saveReportChanges", function(data)
	...
	TriggerClientEvent("mdt:sendNotification", source, "Alterações ao report foram guardadas.")
end)
...
AddEventHandler("mdt:deleteReport", function(id)
	...
	TriggerClientEvent("mdt:sendNotification", source, "Report foi apagado com sucesso.")
end)
...
AddEventHandler("mdt:submitNewReport", function(data)
	...
	exports.oxmysql:insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?)', {data.char_id, data.title, data.incident, charges, author, data.name, data.date,}, function(id)
		...
		TriggerClientEvent("mdt:sendNotification", usource, "Um novo report foi colocado.")
	end)
...
AddEventHandler("mdt:submitNewWarrant", function(data)
	...
	exports.oxmysql:insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', {data.name, data.char_id, data.report_id, data.report_title, data.charges, data.date, data.expire, data.notes, data.author}, function()
		...
		TriggerClientEvent("mdt:sendNotification", usource, "Um novo mandato foi criado.")
	end)
end)
...
AddEventHandler("mdt:deleteWarrant", function(id)
	...
	TriggerClientEvent("mdt:sendNotification", usource, "Mandato foi apagado com sucesso.")
end)
...
AddEventHandler("mdt:getReportDetailsById", function(query, _source)
	...
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` = ?", {query}, function(result)
		if result and result[1] then
			...
			TriggerClientEvent("mdt:closeModal", usource)
			TriggerClientEvent("mdt:sendNotification", usource, "Este report não foi possivel encontrar.")
		end
	end)
end)
	...
	for i= 1, #xPlayers do
		...
		if xPlayer.PlayerData.job.name == 'police' then
			...
			if sendNotification ~= false then
				...
				TriggerClientEvent("mythic_notify:client:SendAlert", source, {type="infom", text="Recebes-te uma nova chamada.", length=5000, style = { ['background-color'] = '#ffffff', ['color'] = '#000000' }})
			end
		end
	end
end)
...
AddEventHandler("mdt:attachToCall", function(index)
	...
	TriggerClientEvent("mdt:sendNotification", usource, "Anexas-te a esta chamada.")
end)
...
AddEventHandler("mdt:detachFromCall", function(index)
	...
	TriggerClientEvent("mdt:sendNotification", usource, "Removes-te de esta chamada.")
end)
...
AddEventHandler("mdt:editCall", function(index, details)
	...
	TriggerClientEvent("mdt:sendNotification", usource, "Tu editas-te esta chamada.")
end)
...
AddEventHandler("mdt:deleteCall", function(index)
	...
	TriggerClientEvent("mdt:sendNotification", usource, "Apagas-te esta chamada.")
end)
...
AddEventHandler("mdt:saveVehicleChanges", function(data)
	...
	MySQL.Async.fetchAll('SELECT * FROM `vehicle_mdt` WHERE `plate` = ?', {data.plate}, function(result)
		...
		TriggerClientEvent("mdt:sendNotification", usource, "Alterações ao veículo guardadas.")
	end)
end)
	...
	if player ~= nil then
		...
		for type,_ in pairs(playerlicenses) do
			if playerlicenses[type] then
				...
				local label = (type:gsub("^%l", string.upper)) .. "s Licença"
