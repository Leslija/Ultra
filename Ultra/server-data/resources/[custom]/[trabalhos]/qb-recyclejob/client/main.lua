...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
    exports['s_target']:RemoveZone(pickupTargetID)
    pickupZone = nil
  else
    pickupZone:destroy()
    pickupZone = nil
...
local function RegisterEntranceTarget()
  ...
  if Config.UseTarget then
    entranceZone = exports['s_target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText('[E] Entrar Armazém', 'left')
      else
...
local function RegisterExitTarget()
  ...
  if Config.UseTarget then
    exitZone = exports['s_target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText('[E] Sair Armazém', 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function DestroyExitTarget()
  ...
  if Config.UseTarget then
    exports['s_target']:RemoveZone(exitTargetID)
    exitZone = nil
  else
    exitZone:destroy()
    exitZone = nil
...
local function GetDutyTargetText()
  local text = onDuty and '[E] Sair de Serviço' or '[E] Entrar de Serviço'
  return text
end
...
local function RegisterDutyTarget()
  ...
  if Config.UseTarget then
    dutyZone = exports['s_target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
...
local function DestroyDutyTarget()
  ...
  if Config.UseTarget then
    exports['s_target']:RemoveZone(dutyTargetID)
    dutyZone = nil
  else
    dutyZone:destroy()
    dutyZone = nil
...
local function RegisterDeliveyTarget()
  ...
  if Config.UseTarget then
    deliveryZone = exports['s_target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
    ...
    deliveryZone:onPlayerInOut(function(isPointInside)
      if isPointInside and carryPackage then
        exports['qb-core']:DrawText('[E] Apanhar Pacote', 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function DestroyDeliveryTarget()
  ...
  if Config.UseTarget then
    exports['s_target']:RemoveZone(deliveryTargetID)
    deliveryZone = nil
  else
...
function RegisterPickupTarget(coords)
  ...
  if Config.UseTarget then
    pickupZone = exports['s_target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      name = pickupTargetID,
      heading = coords.h,
      minZ = coords.z - 1.0,
      maxZ = coords.z + 2.0,
...
RegisterNetEvent('qb-recyclejob:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
    QBCore.Functions.Notify('Entras-te de serviço!', 'success')
    GetRandomPackage()
  else
    QBCore.Functions.Notify('Sais-te de serviço!', 'error')
    DestroyPickupTarget()
  end
...
RegisterNetEvent('qb-recyclejob:client:target:pickupPackage', function()
  ...
  QBCore.Functions.Progressbar('pickup_reycle_package', 'A apanhar pacote..', Config.PickupActionDuration, false, true, {
...
RegisterNetEvent('qb-recyclejob:client:target:dropPackage', function()
  ...
  ScrapAnim()
  DestroyDeliveryTarget()
  QBCore.Functions.Progressbar('deliver_reycle_package', 'A entregar pacote..', Config.DeliveryActionDuration, false, true, {
      disableMovement = true,
      disableCarMovement = true,
      disableMouse = false,
      disableCombat = true
