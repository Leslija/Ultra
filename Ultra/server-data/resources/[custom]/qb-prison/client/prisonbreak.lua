...
RegisterNetEvent('prison:client:SetLockDown', function(isLockdown)
    ...
    if securityLockdown and inJail then
        TriggerEvent("chatMessage", "HOSTAGE", "error", "O nível de segurança mais alto está ativo, fique nos blocos!")
    end
end)
...
RegisterNetEvent('prison:client:PrisonBreakAlert', function()
    ...
    local alertData = {title = "Nova Chamada", coords = {x = coords.x, y = coords.y, z = coords.z}, description = "Fuga Prisão"}
    TriggerEvent("qb-phone:client:addPoliceAlert", alertData)
    TriggerEvent('police:client:policeAlert', coords, "Fuga Prisão")
                ...
                for k in pairs(Gates) do
                    ...
                    if dist < 1.5 then
                        ...
                        if securityLockdown then
                            sleep = 0
                            DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "~r~BLOQUEIO DO SISTEMA")
                        elseif Gates[k].hit then
                            sleep = 0
                            DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "VIOLAÇÃO DO SISTEMA")
