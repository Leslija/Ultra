            ...
            if (data.toggle) {
                ...
                var DotsInterval = setInterval(function() {
                    ...
                    if (loadingProgress == 3) {
                        originalText = "A carregar data"
                        $("#loading-text").html(originalText);
                    }
                    if (loadingProgress == 4) {
                        originalText = "A carregar Personagens"
                        $("#loading-text").html(originalText);
                    }
                    if (loadingProgress == 6) {
                        originalText = "A validar Personagens"
                        $("#loading-text").html(originalText);
                    }
                    if(loadingDots == 4) {
                        $("#loading-text").html(originalText);
...
function setupCharInfo(cData) {
    if (cData == 'empty') {
        $('.character-info-valid').html('<span id="no-char">Cria uma personagem primeiro<br><br>Esta personagem ainda não contém informações.</span>');
    } else {
        var gender = "Man"
        ...
        '<div class="character-info-box"><span id="info-label">Emprego: </span><span class="char-info-js">'+cData.job.label+'</span></div>' +
        ...
        '<div class="character-info-box"><span id="info-label">Nº Telemóvel: </span><span class="char-info-js">'+cData.charinfo.phone+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Nº Conta: </span><span class="char-info-js">'+cData.charinfo.account+'</span></div>');
    }
}
...
function setupCharacters(characters) {
    $.each(characters, function(index, char){
        ...
        setTimeout(function(){
            $('#char-'+char.cid).html('<span id="slot-name"> <i class="fa fa-user" aria-hidden="true" style="color:rgb(0, 248, 185);"></i> '+char.charinfo.firstname+' '+char.charinfo.lastname+'<span id="cid">' + char.citizenid + '</span></span>');
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $("#play-text").html('<i class="fa fa-sign-in" aria-hidden="true"></i> Jogar');
            $("#delete-text").html('<i class="fa fa-trash" aria-hidden="true"></i> Apagar');
            $("#play").css({"display":"block"});
        ...
        if ((selectedChar).data('cid') == "") {
            $(selectedChar).addClass("char-selected");
            setupCharInfo('empty')
            $("#play-text").html('<i class="fa fa-plus" aria-hidden="true"></i> Registar');
            $("#play").css({"display":"block"});
            $("#delete").css({"display":"none"});
            $.post('https://qb-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
...
function setCharactersList() {
    var htmlResult = '<div class="character-list-header"><p>Minhas Personagens</p></div>'
    for (let i = 1; i <= NChar; i++) {
        htmlResult += '<div class="character" id="char-'+ i +'" data-cid=""><span id="slot-name">Lugar Vazio<span id="cid"></span></span></div>'
    }
    htmlResult += '<div class="character-btn" id="play"><p id="play-text">Seleciona um Personagem</p></div><div class="character-btn" id="delete"><p id="delete-text">Seleciona uma Personagem</p></div>'
