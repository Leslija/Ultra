...
if Config.UseTarget then
    CreateThread(function()
        exports['s_target']:AddTargetModel(Config.ATMModels, {
            options = {
                {
    ...
    if cards and cards[1] then
        ...
        for _, v in pairs(Config.ATMModels) do
            ...
            if atm then
                ...
                QBCore.Functions.Progressbar("accessing_atm", "A aceder ao multibanco..", 1500, false, true, {
        ...
        QBCore.Functions.Notify("Por favor visita um banco para pedir um cartão", "error")
    end
end)
...
RegisterNUICallback("removeCard", function(data)
    QBCore.Functions.TriggerCallback('qb-debitcard:server:deleteCard', function(hasDeleted)
        if hasDeleted then
            ...
            QBCore.Functions.Notify('O cartão foi apagado.', 'success')
        else
            QBCore.Functions.Notify('Erro ao apagar cartão.', 'error')
        end
    end, data)
end)
