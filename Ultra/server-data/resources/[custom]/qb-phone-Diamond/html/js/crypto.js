...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Saldo: "+CryptoData.Portfolio+" D-Bit('s)");
    ...
    if (CryptoData.History.length > 0) {
        ...
        $.each(CryptoData.History, function(i, change){
            ...
            var PercentageElement = '<span style="color: #00f8b9;" class="crypto-percentage-change"><i style="color: #00f8b9; font-weight: bolder; transform: rotate(-45deg);" class="fas fa-arrow-right"></i> +('+Math.ceil(PercentageChange)+'%)</span>';
            if (PercentageChange < 0 ) {
                PercentageChange = (PercentageChange * -1);
                PercentageElement = '<span style="color: #ec2b4d; font-weight: bolder;" class="crypto-percentage-change"><i style="color: #ec2b4d; font-weight: bolder; transform: rotate(125deg);" class="fas fa-arrow-right"></i> -('+Math.ceil(PercentageChange)+'%)</span>';
            }
            var Element =   '<div class="cryptotab-course-block">' +
                                '<i class="fas fa-exchange-alt"></i>' +
                                '<span class="cryptotab-course-block-title">Alteração Valor</span>' +
...
function UpdateCryptoData(Crypto) {
    ...
    if (CryptoData.History.length > 0) {
        ...
        $.each(CryptoData.History, function(i, change){
            var PercentageChange = ((change.NewWorth - change.PreviousWorth) / change.PreviousWorth) * 100;
            var PercentageElement = '<span style="color: #00c794; font-weight: bolder;" class="crypto-percentage-change"><i style="color: #00c794; font-weight: bolder; transform: rotate(-45deg); "class="fas fa-arrow-right"></i> +('+Math.ceil(PercentageChange)+'%)</span>';
            if (PercentageChange < 0 ) {
                PercentageChange = (PercentageChange * -1);
                PercentageElement = '<span style="color: #ec2b4d; font-weight: bolder;" class="crypto-percentage-change"><i style="color: #ec2b4d; font-weight: bolder; transform: rotate(125deg); "class="fas fa-arrow-right"></i> -('+Math.ceil(PercentageChange)+'%)</span>';
            }
            var Element =   '<div class="cryptotab-course-block">' +
                                '<i class="fas fa-exchange-alt"></i>' +
                                '<span class="cryptotab-course-block-title">Alteração Valor</span>' +
                                '<span class="cryptotab-course-block-happening"><span style="font-size: 1.3vh;">'+change.PreviousWorth+'€</span> para <span style="font-size: 1.3vh;">'+change.NewWorth+'€</span>'+PercentageElement+'</span>' +
                            '</div>';
...
function RefreshCryptoTransactions(data) {
    ...
    if (data.CryptoTransactions.length > 0) {
        ...
        $.each(data.CryptoTransactions, function(i, transaction){
            var Title = "<span style='color: #00f8b9; font-weight: bolder;'>"+transaction.TransactionTitle+"</span>"
            if (transaction.TransactionTitle == "Sold" || transaction.TransactionTitle == "Transferred") {
                Title = "<span style='color: #ec2b4d; font-weight: bolder;'>"+transaction.TransactionTitle+"</span>"
            }
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (QB.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://qb-phone/BuyCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "QBank", " "+Price+"€,- foi removido do teu saldo!", "#badc58", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Não tens dinheiro suficiente..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Não tens dinheiro suficiente..", "#badc58", 1500);
        ...
        QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Preenche todos os campos!", "#badc58", 1500);
    }
});
        ...
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://qb-phone/SellCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    QB.Phone.Data.PlayerData.money.bank = parseInt(QB.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    QB.Phone.Notifications.Add("fas fa-university", "QBank", " "+Price+"€,- foi adicionado ao teu saldo!", "#badc58", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Não tens D-Bits suficientes..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Não tens D-Bits suficientes", "#badc58", 1500);
            ...
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://qb-phone/TransferCrypto', JSON.stringify({
                    ...
                    if (CryptoData == "notenough") {
                        ...
                        QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Esta Carteira-ID não existe!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Tu transferis-te "+Coins+",- para "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Não podes transferir para ti próprio..", "#badc58", 2500);
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Não tens D-Bits suficientes..", "#badc58", 1500);
        }
...
$(".crypto-action-page-buy-crypto-input-coins").keyup(function(){
    ...
    $(".crypto-action-page-buy-crypto-input-money").html(MoneyAmount+" Euros");
});
...
$(".crypto-action-page-sell-crypto-input-coins").keyup(function(){
    ...
    $(".crypto-action-page-sell-crypto-input-money").html(MoneyAmount+" Euros");
});
