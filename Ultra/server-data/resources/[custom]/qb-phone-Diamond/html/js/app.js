...
QB.Phone.Functions.SetupApplications = function(data) {
    ...
    $.each(data.applications, function(i, app){
        ...
        if ((!app.job || app.job === QB.Phone.Data.PlayerJob.name) && !blockedapp) {
            ...
            var icon = `<img class="geral-icon" src="./img/${app.app}.png"></img>`;
            if (app.app == "meos") {
                    ...
                    $.post('https://qb-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                        ...
                        if (Keys.length > 0) {
                            $.each(Keys, function(i, key){
                                var elem = '<div class="mykeys-key" id="keyid-'+i+'"><span class="mykeys-key-label">' + key.HouseData.adress + '</span> <span class="mykeys-key-sub">Marcar GPS</span> </div>';
