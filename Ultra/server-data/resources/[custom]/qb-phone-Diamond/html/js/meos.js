...
$(document).on('click', '.person-search-result', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Nome: '+ClickedPersonData.firstname+' '+ClickedPersonData.lastname+'</div> <div class="person-search-result-bsn">CSN: '+ClickedPersonData.citizenid+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Data Nascimento: '+ClickedPersonData.birthdate+'</div> <div class="person-search-result-number">Nº Móvel: '+ClickedPersonData.phone+'</div> <div class="person-search-result-nationality">Nacionalidade: '+ClickedPersonData.nationality+'</div> <div class="person-search-result-gender">Sexo: '+Gender+'</div> &nbsp; <div class="person-search-result-apartment"><span id="'+ClickedPersonId+'">Apartmento: '+appartementData.label+'</span> <i class="fas fa-map-marker-alt appartment-adress-location" id="'+ClickedPersonId+'"></i></div> &nbsp; <div class="person-search-result-warned">Mandato: '+IsWarrant+'</div> <div class="person-search-result-driverslicense">Carta de Condução: '+HasLicense+'</div>';
    ...
    if (OpenedPerson === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Nome: '+PreviousPersonData.firstname+' '+PreviousPersonData.lastname+'</div> <div class="person-search-result-bsn">CSN: '+PreviousPersonData.citizenid+'</div>';
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "MDT", "Nº da casa copiado!", "#badc58", 1750);
        ...
        $.post('https://qb-phone/FetchSearchResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, person) {
                    var PersonElement = '<div class="person-search-result" id="person-'+i+'"><div class="person-search-result-name">Nome: '+person.firstname+' '+person.lastname+'</div> <div class="person-search-result-bsn">CSN: '+person.citizenid+'</div> </div>';
                    $(".person-search-results").append(PersonElement);
                    $("#person-"+i).data("PersonData", person);
                });
            } else {
                QB.Phone.Notifications.Add("politie", "MDT", "Não encontrámos resultados!");
                $(".person-search-results").html("");
            }
        });
    } else {
        ...
        $.post('https://qb-phone/FetchVehicleResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, vehicle) {
                    ...
                    var VehicleElement = '<div class="vehicle-search-result"> <div class="vehicle-search-result-name">Veículo</div> <div class="vehicle-search-result-plate">Matrícula: '+vehicle.plate+'</div> <div class="vehicle-opensplit"></div> &nbsp; <div class="vehicle-search-result-owner">Dono: '+vehicle.owner+'</div> &nbsp; <div class="vehicle-search-result-apk">MOT: '+APK+'</div> <div class="vehicle-search-result-warrant">Mandato: '+Flagged+'</div> </div>'
...
AddPoliceAlert = function(data) {
    ...
    if (data.alert.title == "Assistência") {
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData != undefined){
        if (alertData.coords != undefined && alertData.coords != null) {
            ...
            QB.Phone.Notifications.Add("politie", "MDT", "Este alerta não tem uma localização!");
        }
    }else {
        QB.Phone.Notifications.Add("politie", "MDT", "Não existe nenhum alerta disponivel.");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    ...
    QB.Phone.Notifications.Add("politie", "MDT", "Todos os alertas foram apagados!");
});
