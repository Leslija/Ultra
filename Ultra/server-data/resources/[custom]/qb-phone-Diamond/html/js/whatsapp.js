...
QB.Phone.Functions.GetLastMessage = function(messages) {
    ...
    $.each(messages[messages.length - 1], function(i, msg){
        ...
        if(LastMessageData.message == '') 'Hmm, não era suposto conseguir fazer isto...'
    });
...
const monthNames = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "July", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"];
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        ...
        QB.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "Não podes enviar uma mensagem vazia!", "#25D366", 1750);
    }
});
    ...
    if (OpenedChatData.number !== null) {
        if(e.which === 13){
            ...
            if (Message !== null && Message !== undefined && Message !== "") {
                ...
                if (clean == '') clean = 'Hmm, não era suposto conseguir fazer isto...'
                $.post('https://qb-phone/SendMessage', JSON.stringify({
                    ChatNumber: OpenedChatData.number,
                    ChatDate: GetCurrentDateKey(),
                    ChatMessage: clean,
                ...
                QB.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "Não podes enviar uma mensagem para ti próprio!", "#25D366", 1750);
            }
        }
    }
});
...
$(document).on('click', '#send-location', function(e){
    ...
    $.post('https://qb-phone/SendMessage', JSON.stringify({
        ChatNumber: OpenedChatData.number,
        ChatDate: GetCurrentDateKey(),
        ChatMessage: "Partilha Localização",
        ChatTime: FormatMessageTime(),
        ChatType: "location",
    }));
});
    ...
    if (cData) {
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                if (message.message == '') message.message = 'Hmm, não era suposto conseguir fazer isto...'
                var Sender = "me";
