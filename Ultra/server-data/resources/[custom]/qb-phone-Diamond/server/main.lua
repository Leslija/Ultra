...
local WebHook = "https://discord.com/api/webhooks/939943453482188880/1oZIh2MCOGL8DJs15b6Wz6Yix0USxAAEoJbisjGN6-jIY3CLgS1eEzO0KHmIGYxAS-xy"
     ...
     --[[   local garageresult = MySQL.Sync.fetchAll('SELECT * FROM player_vehicles WHERE citizenid = ?', {Player.PlayerData.citizenid})
        if garageresult[1] ~= nil then
            for k, v in pairs(garageresult) do
                ...
                if (QBCore.Shared.Vehicles[vehicleModel] ~= nil) and (Garages[v.garage_id] ~= nil) then
                    v.garage_id = Garages[v.garage_id]
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        invoiceMailData = {
            ...
            message = string.format('Recebes-te uma comissão de %s€ ao %s %s pagar a fatura de %s€.', commission, Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    elseif not SenderPly and Config.BillingCommissions[society] then
        invoiceMailData = {
            sender = 'Faturas',
            subject = 'Fatura paga',
            message = string.format('%s %s fatura paga no valor de %s€', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    end
...
QBCore.Functions.CreateCallback('qb-phone:server:GetCurrentLawyers', function(source, cb)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "strtunners" or Player.PlayerData.job.name == "libertywalk" or
                Player.PlayerData.job.name == "realestate" or Player.PlayerData.job.name == "taxi" or
                Player.PlayerData.job.name == "judge" or Player.PlayerData.job.name == "lawyer" or 
                Player.PlayerData.job.name == "ambulance"  or Player.PlayerData.job.name == "police") and
...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Este nº conta não existe!", "error")
    end
end)
...
RegisterNetEvent('qb-phone:server:sendPing', function(data)
    ...
    if src ~= data then
        ...
        TriggerClientEvent("QBCore:Notify", src, "Não podes dar ping a ti próprio", "error")
    end
end)
...
QBCore.Commands.Add('bill', 'Passar fatura a jogador', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Valor da Fatura'}}, false, function(source, args)
    local biller = QBCore.Functions.GetPlayer(source)
    local billed = QBCore.Functions.GetPlayer(tonumber(args[1]))
    local amount = tonumber(args[2])
    if biller.PlayerData.job.name == "police" or biller.PlayerData.job.name == 'ambulance' or biller.PlayerData.job.name == 'libertywalk' or biller.PlayerData.job.name == 'strtunners' or biller.PlayerData.job.name == 'judge' or biller.PlayerData.job.name == 'lawyer' then
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', source, 'Tem que ser uma quantia válida', 'error')
                end
            else
                TriggerClientEvent('QBCore:Notify', source, 'Não podes passar faturas a ti próprio', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Jogador não está online', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Não tens acesso', 'error')
    end
end)