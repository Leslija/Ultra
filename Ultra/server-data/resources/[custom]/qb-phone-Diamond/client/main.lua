...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.Notify("Não tens telemóvel", "error")
        end
    end)
end
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Telemóvel",
                text = "Não estás a receber nenhuma chamada...",
                icon = "fas fa-phone",
                color = "#e84118",
            },
        })
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        QBCore.Functions.Notify("O teu veículo foi marcado", "success")
    else
        QBCore.Functions.Notify("Este veículo não pode ser localizado", "error")
    end
end)
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    QBCore.Functions.Notify('Localização GPS: '..data.alert.title)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("GPS foi definido para " .. data.HouseData.HouseData.adress .. "!", "success")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS foi definido!', 'success')
end)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
		...
		SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Banco", text = "Conta não existe!", icon = "fas fa-university", color = "#ec2b4d", }, })
...
RegisterCommand('ping', function(source, args)
    local PlayerData = QBCore.Functions.GetPlayerData()
    if not args[1] then
        QBCore.Functions.Notify("Precisas de colocar um Player ID", "error")
    else
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Banco", text = ""..amount.."€ foi adicionado à tua conta!", icon = "fas fa-university", color = "#8c7ae6", }, })
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Mercado",
            text = "Um novo anúncio foi colocado por "..LastAd,
            icon = "fas fa-ad",
            color = "#00f8b9",
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Faturas',
            subject = 'Fatura paga',
            message = 'Fatura foi paga de '..name..' no valor de '..data.amount..'€',
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Faturas',
            subject = 'Fatura recusada',
            message = 'Fatura foi rejeitada de '..name..' no valor de '..data.amount..'€',
        })
    end
end)
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Nova mensagem de "..IsNumberInContacts(SenderNumber).."!",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 1500,
                    },
                ...
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Envias-te mensagem a ti próprio",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 4000,
                    },
...
RegisterNetEvent('qb-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Banco",
                text = ""..amount.."€ foi removido do teu saldo!",
                icon = "fas fa-university",
                color = "#ec2b4d",
                timeout = 3500,
            },
...
RegisterNetEvent('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Telemóvel",
            text = "Tens uma nova sugestão de contato!",
            icon = "fa fa-phone-alt",
            color = "#04b543",
            timeout = 1500,
        },
...
RegisterNetEvent('qb-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Telemóvel",
                text = "Não tens nenhuma chamada recebida...",
                icon = "fas fa-phone",
                color = "#e84118",
            },
        })
...
RegisterNetEvent('qb-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Foste mencionado num Tweet!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#00b6c3", }, })
