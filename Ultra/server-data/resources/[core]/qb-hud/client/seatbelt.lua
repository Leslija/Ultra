...
RegisterKeyMapping('+seatbelt', '[CARRO] Colocar cinto de segurança', 'keyboard', 'B')
...
function toggleSeatbelt(makeSound, toggle)
    if not harnessOn then
        if toggle == nil then
            if seatbeltOn then
                playSound("unbuckle")
                QBCore.Functions.Progressbar("harness_equip", "A remover cinto de segurança..", 1500, false, true, {
                ...
                playSound("buckle")
                QBCore.Functions.Progressbar("harness_equip", "A colocar cinto de segurança..", 1500, false, true, {
                    disableMovement = false,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
        ...
        QBCore.Functions.Progressbar("harness_equip", "A remover cinto de corrida...", 5000, false, true, {
            disableMovement = false,
...
AddEventHandler('seatbelt:client:UseHarness', function(ItemData)
    ...
    if inveh and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
        if not harnessOn then
            QBCore.Functions.Progressbar("harness_equip", "A colocar cinto de corrida...", 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
        ...
        QBCore.Functions.Notify('Não estás em nenhum veículo.', 'error', 3500)
    end
end)
