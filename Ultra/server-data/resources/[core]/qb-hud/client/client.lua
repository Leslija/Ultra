...
RegisterCommand('+onengine', function()
    ...
    if vehicle ~= nil and vehicle ~= 0 and GetPedInVehicleSeat(vehicle, 0) then
        if not IsPauseMenuActive() then
            QBCore.Functions.Notify('Ligas-te o motor!')
...
RegisterCommand('+offengine', function()
    ...
    if vehicle ~= nil and vehicle ~= 0 and GetPedInVehicleSeat(vehicle, 0) then
        if not IsPauseMenuActive() then
            QBCore.Functions.Notify('Desligas-te o motor!')
...
AddEventHandler("EngineAlarm",function()
    if not alarm then
        alarm = true
        local i = 5
		QBCore.Functions.Notify("Veículo Atingindo Dano Crítico.", "error")
        while i > 0 do
...
AddEventHandler("CarFuelAlarm",function()
    if not alarmset then
        alarmset = true
        local i = 5
		QBCore.Functions.Notify("Nível combustível baixo.", "error")
        while i > 0 do
            PlaySound(-1, "5_SEC_WARNING", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
            i = i - 1
            Citizen.Wait(300)
    ...
    while true do
        ...
        if Config.ShowFuel == true then
            if isDriving and IsPedInAnyVehicle(player, true) then
                ...
                local fuellevel = exports["s_fuel"]:GetFuel(veh)
                SendNUIMessage({
                    action = "update_fuel",
                    fuel = fuellevel,
                    showFuel = true
