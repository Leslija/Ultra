...
doc.getElementById('home').addEventListener('mouseover', _ => changeInfo(true, 'home', 'Mostrar todas as animações.'));
...
doc.getElementById('settings').addEventListener('mouseover', _ => changeInfo(true, 'settings', 'Mostra as configurações do menu de animação.'));
...
doc.getElementById('exit').addEventListener('mouseover', _ => changeInfo(true, 'exit', 'Feche o menu de animação.'));
...
doc.getElementById('favorite').addEventListener('mouseover', _ => changeInfo(true, 'favorites', 'Mostre suas animações marcadas.'));
...
doc.getElementById('dances').addEventListener('mouseover', _ => changeInfo(true, 'dances', 'Mostrar animações de dança.'));
...
doc.getElementById('scenarios').addEventListener('mouseover', _ => changeInfo(true, 'scenarios', 'Mostrar animações de cenário.'));
...
doc.getElementById('walks').addEventListener('mouseover', _ => changeInfo(true, 'walks', 'Mostrar animações de caminhada.'));
...
doc.getElementById('expressions').addEventListener('mouseover', _ => changeInfo(true, 'expressions', 'Mostrar expressões faciais.'));
...
doc.getElementById('shared').addEventListener('mouseover', _ => changeInfo(true, 'shared', 'Mostrar animações compartilhadas.'));
...
doc.getElementById('cancel').addEventListener('mouseover', _ => changeInfo(true, 'Cancelar', 'Cancelar a animação em execução no momento.'));
...
doc.getElementById('cancel').addEventListener('click', e => {
    ...
    e.target.style.backgroundColor = "#00b6c3cc";
    setTimeout(() => {
...
doc.getElementById('movement').addEventListener('mouseover', _ => changeInfo(true, 'Movimento', 'Permite movimento durante a reprodução de uma animação (não compartilhado).'));
...
doc.getElementById('loop').addEventListener('mouseover', _ => changeInfo(true, 'Loop', 'Loops animações (sobrepõe a duração nas animações).'));
